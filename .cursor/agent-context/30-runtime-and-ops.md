...
2. **盘后数据报告**（每日 20:00 自动，命令 `main.py post`）：先执行晚间任务（溢价率回填、关注池行情与到价提醒、`review.yaml`→Obsidian），再采集行情、涨跌停、板块等，生成全日盘后报告并推送；报告内含只读的连板断板次日反馈统计，口径为 `T-2 非ST连板≥2 → T-1有交易但不再涨停 → T开盘/收盘反馈`，分别标注断板核验覆盖与反馈行情覆盖，来源缺失不得报作 0；写盘 `post-market.yaml` 后自动同步 `daily_market` 至 SQLite（失败记入 `data/pending_writes.json`，可用 `main.py db sync` 重试），并将盘后 YAML 导出 Obsidian
