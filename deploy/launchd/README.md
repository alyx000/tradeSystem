...
# 均线规则依赖 Tushare；复用主仓库既有凭据文件，不复制凭据或提交到 Git。
# 若 runtime 已有独立 .env，须先核对其 TUSHARE_TOKEN 配置，不覆盖。
if [ ! -e "$RUNTIME_ROOT/scripts/.env" ] && [ ! -L "$RUNTIME_ROOT/scripts/.env" ]; then
  test -f /Users/alyx/tradeSystem/scripts/.env
  ln -s /Users/alyx/tradeSystem/scripts/.env "$RUNTIME_ROOT/scripts/.env"
fi
test -r "$RUNTIME_ROOT/scripts/.env"
