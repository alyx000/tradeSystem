...
- `knowledge add-note / list / draft-from-asset` 已接入真实 service：面向 `knowledge_assets`（**禁止** `teacher_note` 类型写入；API 422 / CLI validation_error），`draft-from-asset` 会生成 `knowledge_asset` observation 与 `TradeDraft`
