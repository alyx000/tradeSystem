...
- `main.py post` 的既有交易日20:00盘后任务同步采集 `raw_data.ipo_effect`：上市1～365/183/90自然日的沪深北A股，剔ST/退市/B股及上市首日，另列一年内剔除上市前5开放日与仅沪深对照；等权涨跌均值/中位、上涨率、±5%尾部、开收盘表现及相对全市场中位差，完整次新成分/来源/缺口随 `post-market.yaml` 和 `daily_market` 信封保存。只读SSE完整日历、开放日且上海16:00后取数；行情唯一、有效行≥4000、有效率/宇宙覆盖≥98%、行情100%匹配身份，硬源失败 `source_failed`，部分缺口 `partial`，休市 `skipped`，不补0。不新增调度、业务表、关注池或计划层写入。
...
- `python3 main.py post --date YYYY-MM-DD` 的盘后主报告同批采集 `raw_data.low_price_effect`：按当日未复权收盘价 `<=10` 元定义低价股并拆 `<=5` 元 / `5～10` 元，剔除 ST/退市与沪深 B 股，统计等权涨跌中位/均值、上涨率、`>=5%`/`<=-5%` 尾部、真实涨跌停率、成交额占比与相对全市场中位差；全市场日线唯一/有效行≥4000/有效率≥98%及非空 ST 名单为硬门，辅助涨跌停/成交额缺口标 `partial`，硬源失败标 `source_failed` 且不得报成样本0；随现有工作日20:00 `today-post` 单一调度落 `post-market.yaml`/`daily_market.raw_data` 并渲染报告，不新增独立 launchd/业务表，不写计划层/关注池，不输出买卖建议；每日复盘 HTML 的④风格区由正式组装器只读同日块自动注入 `data-style-low-price-effect`，`partial/source_failed` 显式进 `ops` 且不补0
- `python3 scripts/main.py low-price-effect backfill --end-date YYYY-MM-DD --days N --input-by USER [--refetch] [--json]` / `low-price-effect trend ...` 是该指标的历史专用入口：精确使用本地开放日历，只补四个必要日期源并原子更新既有盘后信封、复用 `daily_market` 双写，默认生成 CSV/JSON/PNG；完整旧事实默认幂等跳过且刷新降级不覆盖，图表仅连接 `complete` 点；不触发持仓、关注池、监管派生或推送
...
- `python3 main.py ma-breakout daily ...`（4日均线二波尾盘观察池：只保留工作日14:50单次实时快照，CLI仅允许上海当日14:45～15:00且交易日执行；历史龙头宇宙仍只取近60自然日、复盘第5步人工确认的 `leader_tracking`，`trend_leader_pool` 不作默认来源；历史9个完成交易日用 `get_market_daily_quotes`，目标日以 `get_realtime_quotes` 实时价和累计成交额合成临时bar[新浪amount元→Tushare daily千元]，复用 MA4 重新拐头[今日MA4>昨日，昨日<前日<前两日]+累计成交额突破MA5/MA10+快照时未涨停条件；实时行情日期须为当日、最多陈旧10分钟，全批失败重试1次、缺票补取1次，全部不可判=`source_failed`、部分缺失/陈旧=`partial`；按累计成交额降序落 `data/reports/ma-breakout/YYYY-MM-DD.{md,json}` + 钉钉，显式标 `[判断·实时快照]`、尚未收盘确认；`--no-push`落盘不推，`--dry-run`/`--json`不落不推；非当日/非时间窗/非交易日跳过，不再回退上一交易日、不再运行21:35收盘版；不写SQLite/交易计划/关注池，不进`schedule`/APScheduler）
- `python3 main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test [--rule-id RULE_ID] --input-by USER --confirm-real-push [--json]`（可扩展盘中实时监控与独立每3分钟 launchd，额外 09:59 最后补扫；长期单标的规则包括新浪实时 `000001.SH` 上证指数从 `<3955` 站上 `>=3955`，以及同花顺官方实时 `883421.THS` 同花顺全A（沪深）相对昨收单日涨跌幅严格 `<-4.00%`，等于不触发，点位/昨收/更新时间不完整或陈旧时 fail-closed；临时规则含 2026-08-31 `300285.SZ` 国瓷材料严格跌破 67.22 元、2026-08-31～09-02 `688361.SH` 中科飞测严格跌破前 5 个已收盘开放日的前复权 MA5，以及 2026-09-09～09-22 好想你严格突破 11.24 元、品渥食品严格突破 25.89 元、良品铺子严格突破 10.17 元；均线证据不完整时 fail-closed；横截面规则 `limit-up-amount-100b-before-1000` 在上海 `09:30<=quote_time<10:00` 批量扫描最新价达到交易所正式涨停价且当日累计成交额 `>=100亿元` 的 A 股，同股当日只提醒一次，覆盖主板/ST/双创/北交所分币舍入、基金排除、沪深前五开放日与北交所首日无涨跌幅限制排除；部分响应不得伪装完整空池。历史临时规则与下线规则保持既有语义；两类状态独立 pending 先落盘、发送失败同日及窗口后重试、跨日过期，runtime 切换 guard 同时保护；`e2e-test` 仍只验证单标的且须单独授权；不写 SQLite/TradeDraft/TradePlan/持仓/关注池，不构成买卖建议；Mac 休眠期间不触发）
- `python3 main.py intraday-summary run [--dry-run|--no-push] [--json]`（全市场盘中半小时扫描：09:30/13:00 建基线，10:00/10:30/11:00/11:30/13:30/14:00/14:30/15:00 以新浪实时全市场两点快照差分，汇总涨跌扩散、涨幅中位数、成交额增量、4宽基、申万二级与个股波动后推钉钉；只读交易日历，股票清单≥4000、有效行情覆盖≥95%、行情当日且≤10分钟陈旧；缺上一槽位=`partial` 明示未计算，行业/指数缺失只降级对应段；报告先原子落盘，pending 成功后才记 sent，同槽位去重、跨日过期；不写 SQLite/计划层/持仓/关注池，不预测方向、不构成买卖建议；独立 launchd 每分钟轻量 tick、仅槽位后5分钟进入 Python，Mac 休眠错过不补）
...
盘中临时监控补充：`intraday-monitor` 新增美迪西 `688202.SH <87.65元`，2026-09-07～10-06（首尾包含，30自然日）有效，仅交易日盘中每3分钟检查。等于不触发，首次已命中提醒、持续命中去重、恢复后重新跌破提醒；到期停止该规则，不影响其他监控，不写持仓、关注池或计划层。
...
盘中临时监控补充：`intraday-monitor` 新增好想你 `002582.SZ >11.24元`、品渥食品 `300892.SZ >25.89元`、良品铺子 `603719.SH >10.17元`，2026-09-09～09-22（首尾包含，14自然日）有效，仅交易日盘中每3分钟检查。三条均为严格突破，等于不触发；首次已在线上提醒、持续命中去重、回落后重新突破提醒；到期停止对应规则，不影响其他监控，不写持仓、关注池或计划层。
