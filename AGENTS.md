...
2. **盘后数据报告**（每日 20:00 自动，命令 `main.py post`）：先执行晚间任务（溢价率回填、关注池行情与到价提醒、`review.yaml`→Obsidian），再采集行情、涨跌停、板块、北向资金等，生成全日盘后报告并推送，并将 `post-market.yaml` 导出 Obsidian
...
- **核心页面**：仪表盘（`/`）、八步复盘工作台（`/review/:date`，支持预填充 + localStorage 草稿）、信息查询中心（`/search`，跨实体聚合 + Markdown 导出）、老师观点、持仓池、关注池、投资日历
