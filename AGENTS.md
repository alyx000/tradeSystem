...
- `python3 main.py volume-watch ...`（成交额 Top20 板块集中度：`daily` 采集+落库+渲染+钉钉推送 / `trend` 只读趋势；申万二级口径联动 `get_sector_rankings`，落 `daily_volume_concentration`；`daily` 报告额外含**成交额前50 板块区间涨幅排名**[独立取前50→`get_stock_daily_range` 算 5/10/20 日涨幅→板块按板块内涨幅最大个股降序/平手比次大，三档独立榜，落新列 `gain_universe_json`，全 [事实] 守红线]，经只读 API `/api/market/sector-gain-ranking/{date}` 在八步复盘「2.板块」展示）
...
- `python3 main.py research-digest daily ...`（每日研报速读：A股研报评级[巨潮 cninfo `get_research_report_list`] + 美股机构评级[yfinance `upgrades_downgrades`，仅方向变动 init/up/down/reinit] → 鞠磊框架「首次覆盖」加权 Top3 → MD 落盘 + 钉钉；`--dry-run` 仅打印、`--no-llm` 关美股叙事；红线只约束 LLM 叙事不约束取数；每天 22:30 launchd 触发，runner 过滤为 A 股交易日或 A 股交易日前一天，不进 `schedule`/APScheduler）
