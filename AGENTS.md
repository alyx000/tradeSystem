...
- `python3 main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test --input-by USER --confirm-real-push [--json]`（可扩展盘中实时阈值监控与独立每5分钟 launchd；长期规则=新浪实时 `000001.SH` 上证指数从 `<3955` 站上 `>=3955`，首次观察已在线上不补发、持续在线上去重、跌回下方后再次站上可重推；日期限定规则=仅 2026-08-11 监控 `603629.SH` 利通电子严格 `<123.92` 元，首次观察已命中会推送、等于阈值不触发、持续命中去重、恢复后重入，有效期外在取行情前自动排除；科创50规则保持下线；命令用只读 `trade_calendar` 守交易日，行情非当日或陈旧超过10分钟 fail-closed；pending 先原子落状态、发送失败同交易日重试、跨日或退役规则过期；`e2e-test` 必须经用户授权并显式带 `--confirm-real-push`，缺失时在初始化行情源前拒绝；授权后才使用新鲜真实行情与临时测试线推送醒目标注的测试消息，且不读写正式状态；不写 SQLite/TradeDraft/TradePlan/持仓/关注池，不构成买卖建议；Mac 休眠期间不触发）
