...
- `python3 main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test --input-by USER [--json]`（可扩展盘中实时阈值监控：独立 launchd 每5分钟 tick，runner 只放行上海 `09:30-11:30/13:00-15:00`，命令再用只读 `trade_calendar` 守交易日；当前用新浪实时 `000688.SH` 同时监控科创50严格 `<1572` 跌破与“当天先 `<1582` 后 `>=1582`”收复，收复线首次观察已站上只建基线；同源同代码只抓一次，两条规则独立去重、恢复后重入可重推；行情非当日或陈旧超过10分钟 fail-closed；pending 先原子落本地状态、发送失败同交易日重试、跨日过期；`e2e-test` 只在盘中用新鲜真实行情和仅本次测试线发送醒目标注的测试消息，必须带 `--input-by`，不修改正式规则或状态；规则由 `MonitorRule` 注册扩展；不写 SQLite/TradeDraft/TradePlan/持仓/关注池，不构成买卖建议；Mac 休眠期间不触发）
