...
help:
	...
	@echo "  make value-watch-daily / value-watch-daily-dry / value-watch-report - 价值投资条件监控（落库+推送 / 干跑 / 只读快照）"
	@echo "  make morning-brief-daily / morning-brief-daily-dry - 盘前早报（落报告+推送 / 干跑）"
...
# 成交额 Top20 板块集中度监控
volume-watch-daily:
	cd scripts && python3 main.py volume-watch daily

volume-watch-daily-dry:
	cd scripts && python3 main.py volume-watch daily --dry-run
...
morning-brief-daily:
	cd scripts && python3 main.py morning-brief daily

morning-brief-daily-dry:
	cd scripts && python3 main.py morning-brief daily --dry-run

# 价值投资条件监控(红利回撤/卖出阶梯/稀缺周线,teacher_notes#391)
value-watch-daily:
	cd scripts && python3 main.py value-watch daily

value-watch-daily-dry:
	cd scripts && python3 main.py value-watch daily --dry-run

value-watch-report:
	cd scripts && python3 main.py value-watch report
...
# 形态篇选股形态观察清单（主线板块内四条件共振）
pattern-scan-daily:
	cd scripts && python3 main.py pattern-scan daily

pattern-scan-daily-dry:
	cd scripts && python3 main.py pattern-scan daily --dry-run
...
recommend-trace:
	cd scripts && RAINDROP_LOCAL_DEBUGGER="http://localhost:5899/v1/" \
		no_proxy="localhost,127.0.0.1,::1" NO_PROXY="localhost,127.0.0.1,::1" \
		../.venv-raindrop/bin/python main.py recommend daily --dry-run
