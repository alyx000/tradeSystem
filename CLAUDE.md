...
- `python3 main.py intraday-summary run [--dry-run|--no-push] [--json]`（全市场盘中半小时扫描：09:30/13:00 建基线，八个半小时槽位以新浪实时两点快照差分，输出市场宽度、涨幅中位数、成交额增量、宽基/申万二级/个股波动并推钉钉；只读交易日历与全市场覆盖/新鲜度门，缺基线/行业/指数显式 partial，不伪装为 0；报告与 pending 先原子落本地、成功才记 sent，同槽位去重、跨日过期；不写 SQLite/持仓/关注池/计划层、不预测方向；独立 launchd 槽位后5分钟窗口，休眠错过不补）
