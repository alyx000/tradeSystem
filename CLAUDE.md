...
- `python3 main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test --input-by USER [--json]`（可扩展盘中实时阈值监控能力与独立每5分钟 launchd 保留；当前 `DEFAULT_RULES=()`，科创50跌破/收复规则均已下线，没有启用任何生产标的；CLI 不初始化行情 provider，`check` 不访问交易日历/行情/钉钉，状态不存在时不创建、存在时仅将旧 pending 移入 expired 后返回 `no_rules`，避免以后同日复用相同规则 ID 误发；`e2e-test` 不抓行情不推送并非零退出；后续以 `MonitorRule` 注册规则即可恢复双门禁、行情新鲜度 fail-closed、同源同代码去重、持续命中去重/恢复后重入、pending 先落盘及同日重试；不写 SQLite/TradeDraft/TradePlan/持仓/关注池，不构成买卖建议；Mac 休眠期间不触发）
