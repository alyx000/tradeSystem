...
- `python3 main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test [--rule-id RULE_ID] --input-by USER --confirm-real-push [--json]`（可扩展盘中实时监控与独立每5分钟 launchd；当前启用规则仅为新浪实时 `000001.SH` 上证指数从 `<3955` 站上 `>=3955`。金健米业 `600127.SH`、红四方 `603395.SH`、京粮控股 `000505.SZ` 三条断板规则已于 2026-08-20 下线，不再请求行情或推送；科创50规则保持下线。动态涨停价、盘中/收盘阶段、有效期过滤、状态去重与 fail-closed 等通用能力保留；命令用只读 `trade_calendar` 守交易日，行情非当日或陈旧超过10分钟 fail-closed；pending 先原子落状态、发送失败同交易日重试、跨日或退役规则过期；`e2e-test` 可按 `--rule-id` 选择当前生效生产规则，缺少真实推送确认或规则已过期均在初始化行情源前拒绝；授权后才使用新鲜真实行情与临时测试线推送醒目标注的测试消息，且不读写正式状态；不写 SQLite/TradeDraft/TradePlan/持仓/关注池，不构成买卖建议；Mac 休眠期间不触发）
