...
- `python3 main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test [--rule-id RULE_ID] --input-by USER --confirm-real-push [--json]`（可扩展盘中实时监控与独立每5分钟 launchd，额外 09:59 最后补扫；长期单标的规则为新浪实时 `000001.SH` 上证指数从 `<3955` 站上 `>=3955`；横截面规则 `limit-up-amount-100b-before-1000` 在上海 `09:30<=quote_time<10:00` 批量扫描最新价达到交易所正式涨停价且当日累计成交额 `>=100亿元` 的 A 股，同股当日只提醒一次，覆盖主板/ST/双创/北交所分币舍入、基金排除、沪深前五开放日与北交所首日无涨跌幅限制排除；部分响应不得伪装完整空池。历史临时规则与下线规则保持既有语义；两类状态独立 pending 先落盘、发送失败同日及窗口后重试、跨日过期，runtime 切换 guard 同时保护；`e2e-test` 仍只验证单标的且须单独授权；不写 SQLite/TradeDraft/TradePlan/持仓/关注池，不构成买卖建议；Mac 休眠期间不触发）
