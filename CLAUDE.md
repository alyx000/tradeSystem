...
- `python3 main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test [--rule-id RULE_ID] --input-by USER --confirm-real-push [--json]`（可扩展盘中实时监控与独立每5分钟 launchd；长期规则=新浪实时 `000001.SH` 上证指数从 `<3955` 站上 `>=3955`；日期限定规则=2026-08-19～20 监控金健米业 `600127.SH`、红四方 `603395.SH`、京粮控股 `000505.SZ`，以当日新鲜行情 `pre_close` 复用统一涨跌停工具按交易所规则计算涨停价：盘中严格低于只提醒“当前未封涨停，最终以收盘为准”，本机与行情时间均达到15:00且仍低于才额外确认为断板，14:59行情不得冒充收盘；`15:01-15:05`只补取动态规则终态以覆盖相对300秒节拍，阶段进入事件ID且终态入账后才推进确认；等值仍封板，首次已低于会推送、持续命中去重、回封后再开板可重推，有效期外在 provider 前自动排除，前收盘价缺失/非法 fail-closed，三只目标部分不可判为 `partial` 且 CLI 非零退出；科创50规则保持下线；命令用只读 `trade_calendar` 守交易日，行情非当日或陈旧超过10分钟 fail-closed；pending 先原子落状态、发送失败同交易日重试、跨日或退役规则过期；`e2e-test` 可按 `--rule-id` 选择当前生效生产规则，缺少真实推送确认或规则已过期均在初始化行情源前拒绝；授权后才使用新鲜真实行情与临时测试线推送醒目标注的测试消息，且不读写正式状态；不写 SQLite/TradeDraft/TradePlan/持仓/关注池，不构成买卖建议；Mac 休眠期间不触发）
