...
- `python3 main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test [--rule-id RULE_ID] --input-by USER --confirm-real-push [--json]`（可扩展盘中实时监控与独立每5分钟 launchd，额外 09:59 最后补扫；长期单标的规则包括新浪实时 `000001.SH` 上证指数从 `<3955` 站上 `>=3955`，以及同花顺官方实时 `883421.THS` 同花顺全A（沪深）相对昨收单日涨跌幅严格 `<-4.00%`，等于不触发，点位/昨收/更新时间不完整或陈旧时 fail-closed；临时规则含 2026-08-31 `300285.SZ` 国瓷材料严格跌破 67.22 元、2026-08-31～09-02 `688361.SH` 中科飞测严格跌破前 5 个已收盘开放日的前复权 MA5，均等于不触发，均线日历/日线/复权因子不完整时 fail-closed；横截面规则 `limit-up-amount-100b-before-1000` 在上海 `09:30<=quote_time<10:00` 批量扫描最新价达到交易所正式涨停价且当日累计成交额 `>=100亿元` 的 A 股，同股当日只提醒一次，覆盖主板/ST/双创/北交所分币舍入、基金排除、沪深前五开放日与北交所首日无涨跌幅限制排除；部分响应不得伪装完整空池。历史临时规则与下线规则保持既有语义；两类状态独立 pending 先落盘、发送失败同日及窗口后重试、跨日过期，runtime 切换 guard 同时保护；`e2e-test` 仍只验证单标的且须单独授权；不写 SQLite/TradeDraft/TradePlan/持仓/关注池，不构成买卖建议；Mac 休眠期间不触发）
...
盘中临时监控补充：`intraday-monitor` 新增美迪西 `688202.SH <87.65元`，2026-09-07～10-06（首尾包含，30自然日）有效，仅交易日盘中每5分钟检查。等于不触发，首次已命中提醒、持续命中去重、恢复后重新跌破提醒；到期停止该规则，不影响其他监控，不写持仓、关注池或计划层。

盘中临时监控补充：`intraday-monitor` 新增方盛制药 `603998.SH >=11.11元`，2026-09-03～09-16（首尾包含，14自然日）有效，仅交易日盘中每5分钟检查。首次已命中提醒、持续命中去重、回落后重新触达提醒；到期停止该规则，不影响其他监控，不写持仓、关注池或计划层。
