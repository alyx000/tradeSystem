...
`db backup` 不得调用 migrate，避免“为了备份先改库”；目标必须是不存在的新文件，使用 `sqlite3.Connection.backup()` 后 chmod 0600 并在独立只读连接执行 integrity_check。`db migrate` 先验证 `--require-backup`，再调用正式 migrate，最后回查 user_version、六列和三个索引；禁止临时 SQL 或直接修改生产库。
...
store.py 使用 fcntl 文件锁、同目录临时文件、os.replace 和 json.dump(ensure_ascii=False, sort_keys=True)。每个 phase 使用独立 `transaction.json` journal：先持久化 transaction 元数据，再写/替换 raw files，再替换 index，最后才替换带 `commit_state=complete` 的 manifest 并删除 journal。启动时在同一锁内恢复残留 journal；index 已写但 manifest 未写时必须从 journal 重建完成 manifest，index 未写时补写后再完成。show 只读取 complete manifest。对 journal、每个 raw replace、index replace、manifest replace 和 journal delete 分别注入失败，断言下一次运行可恢复、旧完成 manifest 不被破坏、候选不会永久丢失。manifest/journal 绝不包含 base URL 凭据、AK/SK 或 Authorization。
...
- [ ] **Step 2: 验证 skills/commands 同步**
       ...
       rg -n "wechat-teacher-feed" AGENTS.md .agents/skills/INDEX.md \
         ...
         .agents/rules/skills-sync.md docs/commands.md docs/commands.json

Expected：命令在五处真源可检索，CLI smoke 全绿。
