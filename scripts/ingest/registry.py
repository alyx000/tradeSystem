...
INTERFACE_REGISTRY: dict[str, InterfaceConfig] = {
    ...
    "moneyflow_ind_dc": {
        ...
        "enabled_by_default": True,
        "notes": "东财行业板块资金流向（仅行业类型），补行业维度资金确认。",
    },
    "moneyflow_cnt_ths": {
        "interface_name": "moneyflow_cnt_ths",
        "provider_method": "get_concept_moneyflow_ths",
        "stage": "post_core",
        "use_cases": ["post_report", "plan_diagnostics", "history_compare"],
        "params_policy": "trade_date",
        "dedupe_keys": ["trade_date"],
        "raw_table": "raw_moneyflow_cnt_ths",
        "enabled_by_default": True,
        "notes": "同花顺概念板块资金流向，补情绪性板块/题材资金确认。",
    },
    "moneyflow_concept_dc": {
        "interface_name": "moneyflow_concept_dc",
        "provider_method": "get_concept_moneyflow_dc",
        "stage": "post_core",
        "use_cases": ["post_report", "plan_diagnostics", "history_compare"],
        "params_policy": "trade_date",
        "dedupe_keys": ["trade_date"],
        "raw_table": "raw_moneyflow_concept_dc",
        "enabled_by_default": True,
        "notes": "东财概念板块资金流向，补题材维度资金确认。",
    },
    "moneyflow_mkt_dc": {
    ...
    "earnings_forecast": {
        "interface_name": "earnings_forecast",
        "provider_method": "get_earnings_forecast",
        "stage": "post_extended",
        "use_cases": ["watchlist_context", "research", "history_compare"],
        "params_policy": "date_range_day",
        "dedupe_keys": ["start_date", "end_date"],
        "raw_table": "raw_earnings_forecast",
        "enabled_by_default": False,
        "preserve_nonempty_on_empty": True,
        "notes": "业绩预告全市场（forecast_vip，净利万元）。审计 params 为标称单日；provider 实际按公告日回看窗口 [T-EARNINGS_LOOKBACK_DAYS+1, T]（默认 3 自然日）取数，周一窗口覆盖周末公告。由 earnings-digest 显式触发，不进盘后主流程。preserve_nonempty_on_empty：公告一经发布不缩回空，同日返空必是数据源瞬时空窗，落库保留旧非空 payload（仅本接口 opt-in，不改全局覆盖语义）。",
    },
    "earnings_express": {
        "interface_name": "earnings_express",
        "provider_method": "get_earnings_express",
        "stage": "post_extended",
        "use_cases": ["watchlist_context", "research"],
        "params_policy": "date_range_day",
        "dedupe_keys": ["start_date", "end_date"],
        "raw_table": "raw_earnings_express",
        "enabled_by_default": False,
        "preserve_nonempty_on_empty": True,
        "notes": "业绩快报全市场（express_vip，营收/净利单位元、未经审计初步实绩）。窗口语义同 earnings_forecast（标称单日审计 + provider 回看窗口）。由 earnings-digest 显式触发，不进盘后主流程。preserve_nonempty_on_empty 同 earnings_forecast（仅本接口 opt-in）。",
    },
    "stk_limit": {
