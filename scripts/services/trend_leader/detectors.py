...
def is_first_limit_up_acceleration(
    bars: list[dict], code: str, today_limit_up: bool, is_st: bool | None = None
) -> tuple[bool, dict]:
    """首次涨停加速：今日涨停（权威源传入）且近 60 交易日内除今日外无涨停。"""
...
def is_gentle_rise(bars: list[dict], code: str, is_st: bool | None = None) -> tuple[bool, dict]:
    ...
    has_limit = _has_limit_up(window[:-1], _limit_up_threshold(code, is_st))
