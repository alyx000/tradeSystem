...
_DISCLAIMER = (
    ...
    "> 数据时效：涨幅/成交额/尾盘强度为 **T 实时快照**；主线为 **T-1** 口径；"
    "归属概念为扫描时当前快照（不是历史 as-of 快照）；T-1热概念命中为上一交易日资金流口径；"
...
def _concept_names(value, *, allow_empty: bool) -> tuple[list[str], bool]:
    """只接收完整字符串列表；脏类型不降格为可展示事实。"""
...
def _concept_total(value, actual: int) -> int:
    """完整列表契约下，任何不等于实际名称数的总数均退回列表长度。"""
...
def _render_concept_context(card: dict) -> list[str]:
    """分别渲染当前概念归属与 T-1 热概念命中。"""
    stock_status = card.get("stock_concept_status")
    if stock_status == "source_failed":
        membership = "  - [来源状态·归属概念] 概念归属源失败，本次未取得。\n"
    elif stock_status == "missing":
        membership = "  - [来源状态·归属概念] 当前快照暂无可用窄概念。\n"
    elif stock_status == "ok":
        names, valid = _concept_names(card.get("stock_concept_names"), allow_empty=False)
        if valid:
            total = _concept_total(card.get("stock_concept_total"), len(names))
            shown = names[: C.CONCEPT_AFFILIATION_DISPLAY_MAX]
            membership = (
                f"  - [事实·归属概念] {' / '.join(shown)}（当前快照，共{total}个）\n"
            )
        else:
            membership = "  - [来源状态·归属概念] 归属概念数据无效，本次未展示。\n"
    else:
        membership = "  - [来源状态·归属概念] 归属概念状态异常，本次未展示。\n"

    concept_status = card.get("concept_status")
    if concept_status == "source_failed":
        hot = "  - [来源状态·T-1热概念命中] T-1概念资金流源失败，本次无法判断。\n"
    elif concept_status == "member_failed":
        hot = "  - [来源状态·T-1热概念命中] 个股概念归属源失败，本次无法判断。\n"
    elif concept_status == "coverage_failed":
        hot = "  - [来源状态·T-1热概念命中] T-1热概念覆盖数据不完整，本次无法判断。\n"
    elif concept_status == "ok":
        hot_names, valid = _concept_names(card.get("concept_names"), allow_empty=True)
        if not valid:
            hot = (
                "  - [来源状态·T-1热概念命中] "
                "T-1热概念命中数据无效，本次无法判断。\n"
            ...
            shown = hot_names[: C.CONCEPT_HOT_DISPLAY_MAX]
            hot = f"  - [事实·T-1热概念命中] {' / '.join(shown)}\n"
        else:
            hot = "  - [事实·T-1热概念命中] 未命中上一交易日资金流前8窄概念。\n"
    else:
        hot = "  - [来源状态·T-1热概念命中] T-1热概念状态异常，本次无法判断。\n"
    return [membership, hot]


def _stock_concept_failed(card: dict) -> bool:
    status = card.get("stock_concept_status")
    if status == "missing":
        return False
    if status != "ok":
        return True
    return not _concept_names(card.get("stock_concept_names"), allow_empty=False)[1]


def _hot_concept_failed(card: dict) -> bool:
    status = card.get("concept_status")
    if status != "ok":
        return True
    return not _concept_names(card.get("concept_names"), allow_empty=True)[1]
...
def _candidate_block(card: dict, pk_result: dict | None) -> str:
    ...
    lines.extend(_render_concept_context(card))
...
def _degradation_note(scored) -> str:
    ...
    dims = {
        ...
        "归属概念": lambda c: _stock_concept_failed(c),
        "T-1热概念": lambda c: _hot_concept_failed(c),
    ...
    if not hit:
        return ""
    return (f"\n> [判断] 数据降级：**{'/'.join(hit)}** 维度本次取数失败、缺失或数据无效，"
            "相关判断已弱化（非「确定不强」），请知悉。\n")
