...
LEADER_ATTRIBUTE_ROLES = {"趋势中军", "小票弹性（连板）", "20cm", "30cm", "10cm"}
STATUS_ROLES = {"备选", "剔除"}
...
def _build_prompt(proposal: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "{\"rank\": 正整数, \"role\": \"趋势中军|小票弹性（连板）|20cm|30cm|10cm|备选|剔除\", \"reason\": \"中文理由\", \"risk_flags\": [\"风险标签\"]}",
            "role 是最票属性：趋势中军=大成交额/板块容量核心；小票弹性（连板）=情绪连板小票；20cm=创业板/科创板弹性票；30cm=北交所弹性票；10cm=主板涨停/趋势加速票；备选/剔除仅用于证据不足或降级。",
