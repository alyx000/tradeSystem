...
def load_ladder_positions(conn: sqlite3.Connection) -> list[dict]:
    """active 持仓 ∩ LADDER_CODES。身份键 {canonical}:{entry_date}（thesis_id/entry_price
    不进键——补录 thesis、修正成本不换键不重推）；entry_price/entry_date 任一缺失 →
    ...
    for r in rows:
        ...
        out.append({
            ...
            "position_key": None if insufficient else f"{canonical}:{entry_date}",
        })
    return out
