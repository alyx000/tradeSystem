...
（见 tdx_provider 头注释）。所以平均股价与微盘股必须 `call_specific("tdx", ...)` 直连，
其余标准指数走正常 `registry.call` 降级。scanner 一律经本函数取数，避免在编排层
散落路由判断、也避免漏掉 avg_price 的专路。
"""
from __future__ import annotations

from providers.tdx_provider import MICROCAP_CODE
...
def fetch_index_daily(registry, index_code: str, start_date: str, end_date: str):
    ...
    - `avg_price` / `880823.TDX`：`call_specific("tdx", ...)` 绕开 tushare 空成功遮蔽
    ...
    if str(index_code).strip().upper() == MICROCAP_CODE:
        return registry.call_specific("tdx", "get_index_daily_range", MICROCAP_CODE, start_date, end_date)
