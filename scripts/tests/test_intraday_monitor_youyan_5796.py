"""有研硅七个开放日新阈值的严格突破、去重、日期与失败保护。"""
import json
from datetime import date, datetime, timedelta

import pytest

from services.intraday_monitor.rules import DEFAULT_RULES, YOUYAN_SILICON_BREAKOUT_57_96_20260922_1008 as RULE
...
def _setup(tmp_path, day="2026-09-22"):
    ...
    registry, pusher = _Registry(price=57.97), _Pusher()
...
@pytest.mark.parametrize("price,matched", [(46.15, False), (57.95, False), (57.96, False), (57.97, True)])
...
def test_initial_match_dedupe_reentry_and_message(tmp_path):
    ...
    counts = []
    for price in (57.97, 58.00, 57.96, 57.97):
    ...
    assert "有研硅" in pusher.messages[0][1] and "**57.96**元" in pusher.messages[0][1]
