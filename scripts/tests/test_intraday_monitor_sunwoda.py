"""欣旺达七个开放日的严格突破、去重、日期与失败保护。"""
import json
from datetime import date, datetime, timedelta

import pytest

from services.intraday_monitor.rules import DEFAULT_RULES, SUNWODA_BREAKOUT_19_94_20260917_28 as RULE
...
def _setup(tmp_path, day=17):
    ...
    registry, pusher = _Registry(price=19.95), _Pusher()
...
@pytest.mark.parametrize("price,matched", [(19.93, False), (19.94, False), (19.95, True)])
def test_strict_threshold_and_identity(price, matched):
    assert RULE.code == "300207.SZ" and RULE.instrument_name == "欣旺达"
...
def test_initial_match_dedupe_reentry_and_message(tmp_path):
    ...
    counts = []
    for price in (19.95, 20.01, 19.94, 19.95):
    ...
    assert "欣旺达" in pusher.messages[0][1] and "**19.94**元" in pusher.messages[0][1]
