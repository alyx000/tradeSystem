...
对于跨服务层 / 多文件的功能落地（如行业推荐定时推送 16 个 R-G-R），按可独立验证和回滚的功能边界切：

| Commit | 内容 | 边界 |
|---|---|---|
| 1 | 核心 service + 测试（aggregator / 业务核心） | 核心业务单元 |
| 2 | service 编排 + 其他 service 模块（formatter / llm） | 服务编排单元 |
| 3 | 基础设施（新 pusher / 新 provider） | 基础设施单元 |
| 4 | CLI + scheduler + 部署模板 | 入口与部署单元 |
| 5 | 文档同步（SKILL / INDEX / Makefile / README） | 文档契约单元 |
...
- 实施后代码审查：门1 `code-review-gate.md`（`/simplify` 清理 + `/code-review`）→ 门2 `post-dev-codex-review.md`（codex）
- 文档同步：`skills-sync.md`
