...
**呈现铁律**(memory `feedback_review_digest_before_report`):**先打处置标签,再附 review 细节**;**禁止把 subagent 原文整段丢给用户**让其当"reviewer 的 reviewer"。每条必须带以下标签之一,且 1 行内让用户看懂"已处理"还是"等用户拍板":
...
```
[高] 现象:... 为什么是问题:... 修正方向:...   # ← 把 subagent 原文整段抛给用户
```
