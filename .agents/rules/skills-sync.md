...
| `.agents/skills/daily-review/references/html-report-template/assemble_report.py` 或多 Agent HTML chunks / anchors / `sector-labels` / ETF 净申赎契约 | `daily-review/SKILL.md`、`daily-review/references/multi-agent-review.md`、`daily-review/references/html-report-template/README.md`、`INDEX.md` 与 `scripts/tests/test_daily_review_html_report.py`；正式布局核对 7 chunks / 15 anchors，「仓位环境与纪律参考（影子）」和独立「次日推演」不得进入新报告；`sector-labels` 核对唯一可见事实/判断汇总、折叠命中并集、MA144/MA233 与 10/20 共振固定口径、命中/不足计数逐行对账、`complete/partial/missing-data` 三态；ETF 核对净申赎估算额、核心宽基互斥归属与拆分标准化对账 |
...
| 多 Agent HTML `data-rmb-fx-chart`、`data-emotion-leader` 或 `data-emotion-node` 自动模块 | `daily-review/SKILL.md`、`daily-review/references/multi-agent-review.md`、`daily-review/references/html-report-template/README.md`、`INDEX.md` 与 `scripts/tests/test_daily_review_html_report.py`；外汇图只消费已验证的中国货币网即期/1Y C-Swap 事实，按来源日去重并绘制最近 8～15 个同日工作日点，历史不足不补 0、不插值；情绪生命周期核对同日 JSON 的 status/coverage/refresh/计数/晋级/二板候选/前 12 只/source_errors，波段标 `[判断]`；情绪节点固定比较目标日与此前 20 个开放日非 ST 最高连板，高度对比标 `[事实]`、核心启动日节点候选标 `[判断]`，缺口必须显式 `missing-data`；三模块均由组装器注入，chunk 不得手写重复模块 |
...
| `scripts/cli/intraday_monitor.py` 或 `scripts/services/intraday_monitor/` | `market-tasks/SKILL.md` + `INDEX.md` 中 `intraday-monitor` 行 + `AGENTS.md` / `CLAUDE.md` + `deploy/launchd/README.md`；核对长期上证规则保持不变；2026-08-19～20 的三只规则精确为金健米业 `600127.SH`、红四方 `603395.SH`、京粮控股 `000505.SZ`，`threshold_mode=daily_up_limit` 必须用当日新鲜行情 `pre_close` 复用 `utils.price_limit.compute_limit_prices` 按交易所舍入规则动态计算涨停价，盘中严格 `<` 只称当前未封涨停、最终以收盘为准，本机与行情时间都达到15:00且仍 `<` 才额外确认为断板，14:59行情不得冒充收盘；结束分钟晚数秒可进入，并以`15:01-15:05`仅动态规则补确认覆盖相对300秒节拍，事件ID绑定阶段且终态入账后才推进确认；等值不触发、首次已低于会推、持续去重、回封后重入，生效日前后须在 provider 前排除，前收盘价缺失/非法 fail-closed，目标部分不可判须 `partial` 且 CLI 非零退出；科创50保持下线；保留 `MonitorRule` 注册扩展、固定/动态阈值互斥校验、有效日期区间、价格标签/单位、同 provider 同代码去重、上海交易时段与只读交易日历双门禁、实时行情日期/10分钟新鲜度 fail-closed、pending 先落盘且发送成功才记 sent、失败同交易日重试、跨日/退役规则过期与每5分钟独立 launchd；`e2e-test` 可按 `--rule-id` 选择当前生效规则，必须显式 `--input-by` 与 `--confirm-real-push`，缺少确认或规则过期时须在 provider 初始化/日历/行情/推送前拒绝，只认盘中新鲜真实行情且不读写正式状态，真实推送须单独获得用户授权；全链路不写 SQLite/计划层/持仓/关注池、不构成买卖建议 |
