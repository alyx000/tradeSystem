...
| `scripts/cli/intraday_monitor.py` 或 `scripts/services/intraday_monitor/` | `market-tasks/SKILL.md` + `INDEX.md` 中 `intraday-monitor` 行 + `AGENTS.md` / `CLAUDE.md` + `deploy/launchd/README.md`；核对规则注册可扩展、当前科创50 `000688.SH < 1572` 严格跌破与“先 `<1582` 后 `>=1582`”收复两条独立规则、收复线首次观察已站上只建基线、同 provider 同代码去重抓取、上海交易时段与只读交易日历双门禁、实时行情日期/10分钟新鲜度 fail-closed、持续命中去重/恢复后重入、pending 先落盘且发送成功才记 sent、失败同交易日重试与跨日过期、每5分钟独立 launchd；`e2e-test` 必须显式 `--input-by`、只认盘中新鲜真实行情、测试消息醒目标注且不读写正式规则/状态；全链路不写 SQLite/计划层/持仓/关注池、不构成买卖建议 |
