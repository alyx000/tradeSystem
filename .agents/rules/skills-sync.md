...
| `scripts/cli/intraday_monitor.py` 或 `scripts/services/intraday_monitor/` | `market-tasks/SKILL.md` + `INDEX.md` 中 `intraday-monitor` 行 + `AGENTS.md` / `CLAUDE.md` + `deploy/launchd/README.md`；核对上证 `000001.SH` 从 `<3955` 站上 `>=3955` 长期规则保持；2026-08-21～24 自然日期窗口内的科创50 `000688.SH` 规则精确为严格 `>1700`，凯莱英 `002821.SZ` 规则精确为严格 `>172.26`，两者均 `emit_on_initial_match=True`，并由只读交易日历实际覆盖 8 月 21/24 两个交易日，等值不触发、持续去重、回落后重入；旧科创50跌破1572/收复1582及三只断板规则保持下线，新规则均使用独立 rule id；保留 `MonitorRule` 固定/动态阈值、有效日期、同 provider 批量与同代码去重、上海交易时段/交易日历、行情日期/10分钟新鲜度 fail-closed、pending 先落盘、发送成功才记 sent、失败同日重试、跨日/退役过期与每5分钟 launchd；`e2e-test` 必须显式 `--input-by` 与 `--confirm-real-push`，缺确认或规则过期在 provider 前拒绝且不读写正式状态；全链路不写 SQLite/计划层/持仓/关注池、不构成买卖建议 |
