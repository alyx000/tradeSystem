...
| `scripts/cli/intraday_monitor.py` 或 `scripts/services/intraday_monitor/` | `market-tasks/SKILL.md` + `INDEX.md` 中 `intraday-monitor` 行 + `AGENTS.md` / `CLAUDE.md` + `deploy/launchd/README.md`；核对当前 `DEFAULT_RULES=()`、科创50规则已下线，CLI 不初始化 provider，`check` 不访问日历/行情/推送；不存在状态时不创建，存在时只把旧 pending 移入 expired 后返回 `no_rules`，确保同日复用相同规则 ID 也不补发；能力层继续保留 `MonitorRule` 注册扩展、同 provider 同代码去重、上海交易时段与只读交易日历双门禁、实时行情日期/10分钟新鲜度 fail-closed、持续命中去重/恢复后重入、pending 先落盘且发送成功才记 sent、失败同交易日重试、每5分钟独立 launchd；`e2e-test` 必须显式 `--input-by`，无启用规则时不抓行情不推送并非零退出，有规则时只认盘中新鲜真实行情且不读写正式状态；全链路不写 SQLite/计划层/持仓/关注池、不构成买卖建议 |
