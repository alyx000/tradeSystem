...
globs:
  ...
  - scripts/services/low_price_effect_history.py
...
2026-09-13：`market-timing` 默认微盘股从中证2000代理切换到通达信 `880823.TDX`，同步 provider/fetch、六指数清单、市场概览卡片及复盘规范。tdx日线完整800根及窗口起点覆盖校验、浅历史换节点或显式insufficient_depth失败、目标代码验证与失败不换指数；历史中证2000保留身份。同步INDEX、market-tasks/reference、daily-review及相关reference、AGENTS/CLAUDE；CLI签名未变。
...
2026-09-13：`daily-review/references/index-analysis.md` 为各大指数趋势、动能、量价与跨周期解释的详细规范；同步SKILL、第1路流程、HTML模板README与INDEX。核对124开放日MACD预热后最后5日、量额同源/同窗、原11日MA独立保留、同指数跨周期身份、代理不可合并、反向修复和单项缺口；这是Agent生成流程要求，尚非组装器自动注入/硬门，不新增业务写入。规范变更时五处入口需同步，CLI签名未变仍跑smoke。
...
盘中临时规则同步（2026-09-09）：好想你 `002582.SZ >11.24元`、品渥食品 `300892.SZ >25.89元`、良品铺子 `603719.SH >10.17元`，有效期均为 2026-09-09～09-22（首尾包含）；三条均核对严格突破、首次命中、同日去重与恢复重入、有效期前后不取数不推送，CLI 帮助与 E2E 规则选择同步覆盖。全局 launchd 节拍同步从 300 秒改为 180 秒，保留 09:59 补扫。

盘中临时规则同步（2026-09-07）：美迪西 `688202.SH <87.65元`，有效期 2026-09-07～10-06（首尾包含）；核对严格跌破、首次命中、同日去重与恢复重入、有效期前后不取数不推送，CLI 帮助与 E2E 规则选择同步覆盖。
...
- `scripts/analyzers/low_price_effect.py` / `scripts/collectors/market.py` / `scripts/generators/report.py` — 标准盘后低价股赚钱效应的价格分档、样本剔除、覆盖硬门、状态语义、YAML 与报告契约
- `scripts/cli/low_price_effect.py` / `scripts/services/low_price_effect_history.py` — 低价股历史补采、既有盘后信封原子更新、SQLite 双写、降级保护与趋势图断线契约
...
| `.agents/skills/daily-review/references/html-report-template/assemble_report.py` 或多 Agent HTML chunks / anchors / `sector-labels` / 断板反馈 / `data-style-market-effect` / `data-style-low-price-effect` / `data-style-shock-feedback` / ETF 净申赎契约 | `daily-review/SKILL.md`、`daily-review/references/multi-agent-review.md`、`daily-review/references/html-report-template/README.md`、`INDEX.md` 与 `scripts/tests/test_daily_review_html_report.py`；正式布局核对 7 chunks / 15 anchors；断板反馈核对 `s3 data-board-break-feedback` 逐股时间轴与 `s4 data-style-board-break-feedback` 默认可见聚合逐项同源；④自动模块必须从盘后逐股池重算非 ST 10/20/30cm 总数、首板/连板/高度，北交所与 30cm 对账并显示最近最多 5 份可用归档；低价股模块必须只读报告日同日 `raw_data.low_price_effect`，展示 `<=10` 元总体及 `<=5` 元、`5～10` 元分档，严格校验样本与分项闭合，不得回退旧日；严重异动显示 T+1/3/5/10 已实现汇总；`partial/source_failed` 必须进入 ops 且不得写 0；`sector-labels` 核对唯一可见事实/判断汇总、折叠命中并集、MA144/MA233 与 10/20 共振固定口径、命中/不足计数逐行对账、`complete/partial/missing-data` 三态；ETF 核对净申赎估算额、核心宽基互斥归属与拆分标准化对账 |
...
| `scripts/analyzers/low_price_effect.py`、`scripts/collectors/market.py` 的 `low_price_effect` 接线、`scripts/generators/report.py` 的低价股章节、`scripts/cli/low_price_effect.py` 或 `scripts/services/low_price_effect_history.py` | `market-tasks/SKILL.md` + `INDEX.md` + `AGENTS.md` / `CLAUDE.md`；核对复用 `main.py post` 工作日 20:00 单一调度、未复权收盘价≤10元并拆≤5元/5～10元、剔除 ST/退市与沪深 B 股、个股等权与成交额单位、真实涨跌停名单、全市场相对中位、行情代码唯一/有效行≥4000/有效率≥98%硬门、ST 空/失败整块 `source_failed`、涨跌停/成交额辅助缺口只降级 `partial`、YAML/raw-data 双写与报告可见、来源失败不伪装样本0；历史专用入口必须显式 `--input-by`，只调用四个必要源，原子更新既有信封并复用 dual-write，刷新降级不覆盖完整旧事实，趋势只连 complete 点且不补0，不触发推送/持仓/关注池/监管；不写独立业务表/计划层/关注池、不输出买卖建议 |
...
| `scripts/cli/intraday_monitor.py` 或 `scripts/services/intraday_monitor/` | `market-tasks/SKILL.md` + `INDEX.md` 中 `intraday-monitor` 行 + `AGENTS.md` / `CLAUDE.md` + `deploy/launchd/README.md`；核对原有单标的规则语义不漂移；`value_mode=daily_pct_change` 必须用同一实时回包的最新值与昨收自行计算并稳定浮点边界，等于阈值不误触发，昨收/时间/布局缺证据 fail-closed；动态 `previous_close_ma` 必须由只读开放日历精确锁定样本、日线与复权因子逐日对齐并前复权，缺证据 fail-closed；横截面 `limit-up-amount-100b-before-1000` 精确为上海 `09:30<=quote_time<10:00` 最新价达到正式涨停价且累计成交额 `>=100亿元`，覆盖主板/ST/双创/北交所分币舍入、基金排除、沪深前五开放日及北交所首日无涨跌幅限制排除、同股当日一次；部分响应/时点不可判必须 `partial` 且已确认命中仍推，重复行情/多 active rule 不漏不重；保留行情失败重试一次、独立 pending/sent 状态、失败同日及窗口后重试、跨日过期、每3分钟+09:59补扫与双状态 transition guard（部署时分两次调用兼容旧 guard）；`e2e-test` 仍只验证单标的且必须单独授权；全链路不写 SQLite/计划层/持仓/关注池、不构成买卖建议 |
...
| 仓库维护工作流、CLI/API 对齐、巡检、文档/索引同步 | `repo-maintenance-workflows/SKILL.md` 保留边界与验证，细节按需读取其 `references/maintenance-checklist.md`；诊断、Review 与巡检默认只读，授权修复按范围验证 |
