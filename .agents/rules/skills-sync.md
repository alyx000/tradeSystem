...
| `.agents/skills/daily-review/references/html-report-template/assemble_report.py` 或多 Agent HTML `exposure` / `sector-labels` / ETF 净申赎契约 | `daily-review/SKILL.md`、`daily-review/references/multi-agent-review.md`、`daily-review/references/html-report-template/README.md`、`INDEX.md` 与 `scripts/tests/test_daily_review_html_report.py`；`exposure` 核对 `shadow/fallback/conflicted/no_data` 状态机、严格报告日来源、candidate-only 只触发市场事实主导的低置信保守档位、广度/量能/结构受控状态到 tier 的机械映射；`fallback+cautious` 默认可见区必须恰好为结论/上行门/下行门/缺口复核 4 条，market/cognition/teacher 与同名 evidence 合并后默认折叠，`not-read/unreconciled` 组合对账作为第 4 项受控 `portfolio` evidence 折叠保留，折叠容器/summary/正文/四项证据及其实质后代的显式或祖先 `hidden` 均拒绝；继续核对成交额/涨跌家数/MA20/MA5/跌停结构化门、连续 `trade_calendar` 复核脊柱，以及只读 `data/trade.db` 对 `daily_market`、`trade_calendar` 和任一 mode `unreconciled` 组合五项事实的外部逐字段对账，谨慎 fallback 不得在 context 已加载时改报 `not-read`；`holdings` 仅证明组装时当前状态，不冒充报告日历史快照；命中只记证据不自动改档或持仓；整个章节（含折叠证据）继续执行比例/操作指令同义词，并拒绝任何不归属 Claim/已知 role 的默认可见正文；`sector-labels` 核对唯一可见事实/判断汇总、折叠命中并集、MA144/MA233 与 10/20 共振固定口径、命中/不足计数逐行对账、`complete/partial/missing-data` 三态和 ops 可见缺口；ETF 核对“份额变化×收盘价”只称净申赎估算额，核心宽基按单一指数族互斥归属，A50 不吞 A500、中证500 不吞 A500ETF，行业/因子衍生不混宽基，拆分已知则标准化还原并计入合计，主题关键词重叠只作独立观察 |
...
| `scripts/cli/intraday_monitor.py` 或 `scripts/services/intraday_monitor/` | `market-tasks/SKILL.md` + `INDEX.md` 中 `intraday-monitor` 行 + `AGENTS.md` / `CLAUDE.md` + `deploy/launchd/README.md`；核对长期规则为新浪实时 `000001.SH` 上证指数从 `<3955` 站上 `>=3955`（`direction=above`、`inclusive=True`、`emit_on_initial_match=False`、动作“站上”），日期限定规则仅在 2026-08-11 监控 `603629.SH` 利通电子严格 `<123.92` 元（`direction=below`、`inclusive=False`、`emit_on_initial_match=True`），生效日前后须在 provider 取行情前排除，科创50规则保持下线；保留 `MonitorRule` 注册扩展、有效日期区间校验、价格标签/单位、同 provider 同代码去重、上海交易时段与只读交易日历双门禁、实时行情日期/10分钟新鲜度 fail-closed、各规则首次命中策略、持续命中去重/恢复后重入、pending 先落盘且发送成功才记 sent、失败同交易日重试、跨日/退役规则过期与每5分钟独立 launchd；`e2e-test` 必须显式 `--input-by` 与 `--confirm-real-push`，缺少确认时须在 provider 初始化/日历/行情/推送前拒绝，只认盘中新鲜真实行情且不读写正式状态，真实推送须单独获得用户授权；全链路不写 SQLite/计划层/持仓/关注池、不构成买卖建议 |
