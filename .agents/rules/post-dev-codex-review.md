---
description: 实质性代码改动完成后，必须主动跑 Codex 原生 adversarial-review，并按 6 条二值结束条件收敛，防止无限循环
alwaysApply: true
---
...
> 2. 门1 高优先级问题修完后 → **再**跑本规则门2（Codex 原生 adversarial-review，独立审查 + 6 条结束条件 + 3 轮上限）；
...
1. **代码改动完成、pytest / make check-scripts 跑通后**，立刻跑 **Codex 原生 adversarial-review（内置 reviewer，前台）**。它**自己读 git 状态**，**无需手工拼 diff / 贴文件路径**，输出结构化分级 findings（`verdict` + 按严重度排序的 `- [severity] title (file:line)` + body / confidence / recommendation + `next_steps`）——这套结构和本规则"严重/中等/轻微 + 现象/为什么/修正方向"契约几乎 1:1。

   `/codex:review` / `/codex:adversarial-review` 两个 slash 命令是 `disable-model-invocation: true`，**Claude 自己不能触发**；直接 Bash 调它们的引擎（companion 路径随插件升级漂移，用 glob 取最新，别硬编码版本号）：

   ```bash
   COMPANION="$(ls -t /Users/alyx/.claude/plugins/cache/openai-codex/codex/*/scripts/codex-companion.mjs 2>/dev/null | head -1)"
   # 改动还在工作区未提交（默认 auto/working-tree，reviewer 自读 git）：
   node "$COMPANION" adversarial-review --wait "重点:bug/行为回归/边界/测试缺口/类型与接口一致性/安全"
   # 改动已提交在 feature 分支上（审分支 diff）：
   node "$COMPANION" adversarial-review --wait --base main "重点:bug/行为回归/边界/测试缺口/类型与接口一致性/安全"
   ```

   - **`--wait` = 前台同步返回**，满足"卡门 review 必须前台"；**绝不传 `--background`**——见下方"为什么 review 必须前台"。
   - 用 `adversarial-review`（而非中性的纯 `review`）：对抗式立场（"尽力证明这次改动不该上线"）+ 支持传入本项目聚焦词（bug/行为回归/边界/测试缺口/类型与接口一致性/安全），最贴合"独立第二意见·避开同源 bias"。
   - 若 companion 报 codex 缺失 / 未鉴权 → 停下让用户 `/codex:setup`。
   - **严重度映射**（原生 4 档 → 本项目 3 档处置契约）：`critical` + `high` → **严重**；`medium` → **中等**；`low` → **轻微**；`verdict: needs-attention` 且含 critical ↔ 结束条件 5「不可推进 → 重做再审」。

   > **方案级（设计文档）review 不走这条**：原生 reviewer 的 target 只有 `uncommittedChanges` / `baseBranch` 两种，本质是 git diff 代码审查器，**没有"审一份设计 prose"的模式**；方案通常在对话里或 `~/.claude/plans/*.md`（仓库外）根本没有 diff 可指。需要 codex 出**方案级独立意见**时仍走 freeform `Agent(subagent_type="codex:codex-rescue")`，把设计提案喂给 codex 挑战方案选择——这条只规定"codex 审方案用哪条机制"，**不替代** [`implementation-plan.md`](implementation-plan.md)「方案 Review（CreatePlan 后强制）」那道 Claude 侧 Explore subagent 门（两者互补：Explore 是同源自审，codex 是跨源第二意见）。详见下方「与方案级 codex review 的区分」。
...
- **代码级 review 用原生 `adversarial-review --wait` 跑前台**:`--wait` 强制同步返回,不进 BG 路径,根本不存在被误判转 BG 的漂移。(方案级 review 走 freeform codex-rescue 时,prompt 里仍须显式写"前台运行 / foreground / 不要 --background",防 rescue 子 agent 把方案审查误判为"复杂"而转 BG。)
...
| 执行入口 | freeform `Agent(subagent_type="codex:codex-rescue")` | Codex 原生 `adversarial-review --wait`（内置 reviewer，自读 git） |
| 内容 | 方案文档全文（无 git diff，喂不进原生 reviewer） | git 工作区 / 分支 diff（reviewer 自读，无需手工拼 diff） |
