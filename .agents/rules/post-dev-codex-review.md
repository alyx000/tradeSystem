...
1. **代码改动完成、pytest / make check-scripts 跑通后**，立刻跑 **Codex 原生 adversarial-review（内置 reviewer，前台）**。它**自己读 git 状态**，**无需手工拼 diff / 贴文件路径**，输出结构化分级 findings（`verdict` + 按严重度排序的 `- [severity] title (file:line)` + body / confidence / recommendation + `next_steps`）——这套结构和本规则"严重/中等/轻微 + 现象/为什么/修正方向"契约几乎 1:1。
   ...
   > **方案级（设计文档）review 不走这条**：原生 reviewer 的 target 只有 `uncommittedChanges` / `baseBranch` 两种，本质是 git diff 代码审查器，**没有"审一份设计 prose"的模式**。只有用户明确要求或任务另有审查要求时，才按当前环境可用方式取得方案级独立意见；本规则不强制为方案额外启动 Agent。
...
- **代码级 review 用原生 `adversarial-review --wait` 跑前台**：`--wait` 强制同步返回，不进 BG 路径。若另行执行方案级 review，其入口与运行方式以当次审查要求为准。
...
- 是否把实现、测试或大规模生成交给 BG，由当前执行环境、用户要求与任务需要决定；本规则只要求代码 review 使用前台 `--wait`。
...
|---|---|---|
| 触发 | 用户明确要求或任务另有方案审查要求 | 实质性代码改动完成后 |
| 执行入口 | 按当前环境可用的方案审查方式 | Codex 原生 `adversarial-review --wait`（内置 reviewer，自读 git） |
| 内容 | 方案文档全文（无 git diff，喂不进原生 reviewer） | git 工作区 / 分支 diff（reviewer 自读，无需手工拼 diff） |
| 关键工件 | plan v2 + 修订对照表 | 修订后的代码 + TODO + 测试 |
| 结束条件 | 以当次方案审查要求为准 | 6 条（本节） |
| 上限 | 以当次方案审查要求为准 | 3 轮（代码改动有边际成本） |
