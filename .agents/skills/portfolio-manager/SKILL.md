...
```bash
python3 main.py db holdings-add ... --entry-date 2026-07-01 --input-by manual [--thesis-id <id>]
python3 main.py db holdings-remove ... --input-by manual
python3 main.py db holdings-refresh --date YYYY-MM-DD   # 现价回填（原 make holdings-refresh）
python3 main.py db holdings-import-yaml --input-by manual   # 遗留 YAML 一次性导入 DB
...
- **所有 holdings 写入口带 `input_by` 审计**：CLI（`holdings-add` / `holdings-remove` / `holdings-import-yaml`）`--input-by` 必填；API 未传时服务端缺省 `web`；系统内部路径落 `system`。
- **`--entry-date` 语义**：新建持仓未传时落当日；更新已有持仓未传时保留原值。存量 `entry_date=NULL` 的历史行**不要猜成今天**——留空等用户补录真实建仓日。
- **API DELETE = soft close**：`DELETE /api/holdings/{hid}` 置 `status='closed'`，行保留使 `input_by` 审计可落，不物理删除；closed 行不可变——`PUT` 禁改 `status`（422），改 closed 行其他字段返回 409；重新开仓走 `POST` 新行。
- **value-watch 卖出阶梯依赖持仓身份完整**：`value-watch` 按 `entry_price` + `entry_date` 计算价格涨幅与事件身份键（`canonical:entry_date:holding_id`）；两者任一缺失该持仓标 `insufficient_identity`，只在报告呈现、不推送、不猜测。
