...
| `market-tasks` | `python main.py sector-correlation daily [--date] [--windows 20,60] [--top-industries 15] [--top-concepts 10] [--activity-days 10] [--indices a,b] [--no-concept] [--dry-run]` | 板块相关性日报（Tushare 主源：多日活跃选板块[行业按成交额 / 概念按换手率] + 4 指数 → 双窗 20/60 原始相关 + 剔大盘超额相关 + β → 落 `sector_correlation_daily` + 钉钉；报告含板块×大盘同向/逆向、联动榜、反向榜双窗对照；`--dry-run` 仅打印） |
