...
| `daily-review` | `python .agents/skills/daily-review/references/html-report-template/assemble_report.py TMP YYYY-MM-DD [--emotion-history-dir PATH]` | 每日复盘 HTML 固定按 `trade_calendar.date` 最近最多 20 个开放日对齐情绪日报，注入 `data-emotion-height-chart` 静态 SVG：逐日展示非 ST 二板及以上最高连板并附折叠数据表；至少 2 个有效日才绘图，确认无符合项才记 0，缺失/损坏/不可判日期保留为空点、断线且整体标 `partial`，不得补 0 或插值；chunk 自行注入、日期乱序或点数/状态不一致会被组装器硬拒绝 |
