...
| `market-tasks` | `python main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test --input-by USER [--json]` | 可扩展盘中实时阈值监控能力与每 5 分钟 launchd 调度保留；当前 `DEFAULT_RULES=()`，科创50跌破/收复规则均已下线，没有启用任何生产标的。CLI 不初始化行情 provider，`check` 不访问交易日历/行情源/钉钉；状态文件不存在时不创建，存在时只把旧 pending 移入 expired，防止同日复用相同规则 ID 后误发，其余历史保留，随后返回 `no_rules`。`e2e-test` 无启用规则时返回 `no_rules`、非零退出且不发送。后续通过 `MonitorRule` 注册新规则即可恢复双门禁、行情新鲜度 fail-closed、pending 先落盘、同日重试、持续命中去重及恢复后重入等通用能力；不写 SQLite、计划层、持仓或关注池 |
