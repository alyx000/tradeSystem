...
| `market-tasks` | `python main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test --input-by USER --confirm-real-push [--json]` | 可扩展盘中实时阈值监控与每 5 分钟 launchd；长期规则监控新浪实时 `000001.SH` 上证指数从 `<3955` 站上 `>=3955`，首次观察已在线上不补发、持续在线上去重、跌回下方后再次站上可重推；日期限定规则仅在 2026-08-11 监控 `603629.SH` 利通电子严格 `<123.92` 元，首次观察已命中会推送、持续命中去重、恢复后重入，生效日前后在取行情前自动排除；科创50规则保持下线。保留上海交易时段+只读交易日历双门禁、当日且不超过10分钟行情新鲜度、pending 先落盘、失败同日重试和跨日/退役规则过期；`e2e-test` 必须经用户授权并显式带 `--confirm-real-push`，缺失时在初始化行情源前拒绝，授权后才使用临时测试线且不读写正式状态；不写 SQLite、计划层、持仓或关注池 |
