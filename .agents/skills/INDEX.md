...
| `market-tasks` | `python main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test [--rule-id RULE_ID] --input-by USER --confirm-real-push [--json]` | 可扩展盘中实时阈值监控与每 5 分钟 launchd；长期监控新浪实时 `000001.SH` 上证指数从 `<3955` 站上 `>=3955`；临时规则在 2026-08-21～24 自然日期窗口内监控 `000688.SH` 科创50严格 `>1700` 及 `002821.SZ` 凯莱英严格 `>172.26`，结合只读交易日历实际覆盖 8 月 21 日与 24 日两个开放交易日，首次已命中会推、等值不触发、持续命中去重、回落后再次突破可重推。三只断板规则及旧科创50跌破/收复规则保持下线。保留固定/动态阈值、有效期过滤、同代码去重、当日且不超过10分钟行情新鲜度、pending 先落盘、失败同日重试和跨日/退役规则过期；`e2e-test` 缺确认或规则过期均在 provider 前拒绝；不写 SQLite、计划层、持仓或关注池 |
