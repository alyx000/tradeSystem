...
| `market-tasks` | `python main.py post --date` | 盘后任务采集；20:00 同批沉淀全球权益/中国风险映射/商品/VIX/中美债券与人民币即期/C-Swap 至 `post-market.yaml.raw_data`，供后续复盘优先读取；A50/黄金/原油/铜优先取不晚于目标日的日期化日线（A50 东财期指有限重试→明确标注的 XIN9 指数代理，商品东财→新浪），日期化源均失败才降级为无来源日的 fetch-only 实时快照；同批新增 `low_price_effect`：未复权收盘价≤10元（拆≤5元/5～10元）、剔除 ST/退市/B股，统计涨跌中位/均值、上涨率、±5%尾部、真实涨跌停率、成交额占比及相对全市场中位差，行情/ST硬源失败=`source_failed`、辅助缺口=`partial`，不把来源失败报成0，不写计划层或独立业务表 |
| `market-tasks` | `python3 scripts/main.py low-price-effect backfill --end-date YYYY-MM-DD [--days 10] --input-by USER [--refetch] [--output-dir DIR] [--json]` / `low-price-effect trend --end-date YYYY-MM-DD [--days 10] [--output-dir DIR] [--json]` | 低价股赚钱效应历史专用入口：按本地开放日历只补 ST/涨停/跌停/全市场日线四源，原子更新既有 `post-market.yaml.raw_data.low_price_effect` 并复用 `daily_market` 双写；已有 complete 幂等跳过，强刷降级不覆盖旧事实；默认生成 CSV/JSON/PNG，只有 complete 点连线，partial/source_failed 断线且不补 0；全程不刷新持仓/关注池/监管、不推送 |
...
| `market-tasks` | `python main.py morning-brief daily [--date YYYY-MM-DD] [--dry-run\|--no-push]` | 盘前早报：隔夜行情快照（标普/纳指/道指/金龙[PGJ代理]/黄金/原油/铜，逐标的失败隔离标缺）+ 海外/国内要闻（复用 macro-flash 金十采集器，窗口=上一交易日 20:00→今 08:00，`morning_brief.keywords` 双主题词表 + important「其他要闻」兜底，每主题 15/其他 5 条）+ 上市公司公告（新 capability `get_market_announcements_range` 直连巨潮全市场分页 + 页数/时间预算 + 时间早停，窗口=上一交易日 15:00→今 08:00，噪音排除→七组分类→同股同组去重→每组 10 条）→ 落 `data/reports/morning-brief/YYYY-MM-DD.md`（原子写）+ 钉钉（18KB 整块截断）；金十失败=`source_failed` 落失败报告+推告警+非零退出，公告/单标的行情失败只降级对应段（`partial` 缺口在报告头显式列出）；非交易日守卫跳过（dry-run 豁免）；三档=裸[落报告+推]/`--no-push`[落报告不推]/`--dry-run`[仅打印]；转述事实层无 LLM 生成段、不写 SQLite 业务表/计划层/关注池；工作日 08:00 per-task launchd，不进 `schedule`/APScheduler |
...
| `market-tasks` | `python main.py sector-correlation daily [--date] [--windows 5,20,60] [--top-industries 15] [--top-concepts 10] [--activity-days 10] [--indices a,b] [--no-concept] [--dry-run] [--no-push] [--input-by USER]` | 板块相关性日报（Tushare 主源：多日活跃选板块[行业按成交额 / 概念按换手率] + 4 指数 → 多窗 5/20/60 原始相关 + 剔大盘超额相关 + β → 落 `sector_correlation_daily` + 钉钉；报告含近5日联动榜(短期共振)、板块×大盘同向/逆向、结构联动榜(60日)、反向榜双窗对照(5日/60日)；`--dry-run` 仅打印）同日同值涨跌幅重复行折叠且审计计数；同日冲突序列剔除并展示 partial 缺口。`daily --no-push --input-by USER` 落库并打印、不推送，`--dry-run` 不落库不推送；请求者写入快照 meta。 |
...
| `market-tasks` | `python main.py emotion-leader daily [--date YYYY-MM-DD] [--lookback-days N] [--max-rows N] [--full-refresh] [--dry-run] [--no-push] [--json]` | 情绪核心生命周期监控：只读 `daily_market.raw_data` 重建非 ST 连板，二板候选、三板或当日高度前二晋级，并合并复盘第 5 步人工确认核心；以启动日前收盘为基准，用前复权 OHLC 统计最大/区间涨幅、距峰值和最高连板数，波段标签明确为 `[判断]`；固定比较目标日与此前 20 个开放日的非 ST 最高连板数，严格创新高时输出打开高度核心的启动日，前者标 `[事实]`、启动日节点候选标 `[判断]`，窗口/启动日缺口为 `missing_data`；默认由目标日前最近同口径日报增量刷新上期活跃/本期新增/重新触发者，已归档未触发者只缓存保留并标指标日期，`--full-refresh` 强制全量；目标日涨跌停事实不完整=`source_failed`，历史/行业/单票行情复权缺口=`partial`，不把失败报成空池；裸 daily 原子落 `data/reports/emotion-leader/YYYY-MM-DD.{md,json}`+推，`--no-push` 只落，`--dry-run`/`--json` 零写入零推送；不写 SQLite/持仓/关注池/计划层 |
| `daily-review` | `python .agents/skills/daily-review/references/html-report-template/assemble_report.py TMP YYYY-MM-DD [--emotion-history-dir PATH] [--daily-dir PATH] [--shock-feedback-report PATH]` | 每日复盘 HTML 固定按 `trade_calendar.date` 最近最多 20 个开放日对齐情绪日报，注入 `data-emotion-height-chart` 静态 SVG；④另从最近最多 5 份盘后 YAML 逐股重算非 ST 涨停总量、首板/连板、10/20/30cm 和北交所身份，并读取同日严重异动 Markdown 展示 T+1/T+3/T+5/T+10 已实现中位数与正收益样本；同时严格读取报告日 `raw_data.low_price_effect` 注入 `data-style-low-price-effect`，展示 `<=10` 元总体及 `<=5` 元、`5～10` 元分档的等权赚钱效应、成交额占比与相对全市场中位差。来源失败或部分态必须进入 `ops`，不得补 0；chunk 自行注入、日期乱序或总分项不闭合会被组装器硬拒绝 |
| `market-tasks` | `python main.py tail-scan daily [--date YYYY-MM-DD] [--min-pct 7] [--min-amount 20] [--dry-run] [--no-push] [--no-llm]` | 盘中尾盘强势股扫描（无状态只读观察清单，14:40 单次快照）：全市场实时行情（`get_realtime_quotes`，单点脆弱源失败重试一次仍失败 → `source_failed`）→ 四条件筛选（涨幅>7% ∩ 非ST ∩ 成交额>20亿 ∩ 未涨停，全 [事实]；涨停按实时价是否触及板块制度、前收盘价及交易所分币舍入后的正式涨停价判定，在事实卡/PK前剔除；上市后前5个开放日不套用常规理论涨停价，上市日或所需交易日历无法确认时 `source_failed`）→ 四维事实卡（逻辑：T-1 主线申万二级 Top-K + 同花顺概念资金流 T-1 Top-M + 老师观点命中；三位一体：候选池内涨幅名次 + 指数背景；节奏：近5日涨幅/MA上方/连涨天数/半日放量追平昨日全日节奏代理 `first_surge`；节点：距前高/是否破前高，单维度取数失败只降级该维度不中断整批）+ 产业逻辑增强（每票 `[事实·主营]`：provider capability `get_stock_business_profiles`，Tushare `stock_company` 主源/AkShare `stock_zyjs_ths` 补缺，为扫描时当前公开静态资料、非历史 as-of，摘要优先级=`main_business`>`introduction`>`business_scope`；`[判断·产业链位置]`：仅基于申万二级+主营摘要+产品受控归纳；近30自然日催化：只读 `teacher_notes` 精确代码/慧博精确名称/`industry_info` 基于申万二级/主营摘要/产品/已验证概念标签受控匹配，按 [事实]/[老师观点]/[研报观点]/[来源陈述] 分层，失败仅降级对应维度）→ 粗权重分仅用于 PK 强池截断（`PK_POOL_MAX=12`）与排序破平，**不进 PK prompt**；PK 只喂边界受控字段（产业链位置显式保留程序 [判断]，近期催化保留事实/观点/来源标签）→ `--no-llm` 未关时跑 LLM 两两 PK 循环赛（180s 预算熔断/无效场率>50%熔断/红线过滤，镜像 board-break）→ 本地 MD 全量落盘，钉钉超长时限制为 ≤18000 UTF-8 bytes、最多前12个完整候选块并附完整报告路径；三档=裸[落盘+推]/`--no-push`[落盘+仅打印]/`--dry-run`[只打印不落盘不推送]；核心源失败落失败报告 + 推告警 + 非零退出；无池无状态，不写交易计划/关注池 |
| `market-tasks` | `python main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test [--rule-id RULE_ID] --input-by USER --confirm-real-push [--json]` | 可扩展盘中监控与每 3 分钟 launchd（另有 09:59 最后补扫）；长期规则监控上证指数从 `<3955` 站上 `>=3955`，并用同花顺官方实时 `883421.THS` 监控同花顺全A（沪深）相对昨收的单日涨跌幅严格 `<-4.00%`（等于不触发，点位/昨收/更新时间不完整或陈旧时 fail-closed）；临时规则含 2026-08-31 国瓷材料严格跌破 67.22 元与 2026-08-31～09-02 中科飞测严格跌破前 5 个已收盘开放日的前复权 MA5，均线证据不完整时 fail-closed；横截面规则在 `09:30<=quote_time<10:00` 扫描最新价达到正式涨停价且累计成交额 `>=100亿元` 的 A 股，同股当日只推一次，覆盖板块/ST以及沪深前五开放日、北交所首日无涨跌幅限制口径；部分行情不得伪装完整空池。历史临时规则与下线规则保持既有语义；两类状态均 pending 先落盘、失败同日重试、跨日过期，runtime 切换 guard 同时保护；不写 SQLite、计划层、持仓或关注池 |
| `market-tasks` | `python main.py intraday-summary run [--dry-run\|--no-push] [--json]` | 全市场盘中半小时扫描：09:30/13:00 建基线，10:00/10:30/11:00/11:30/13:30/14:00/14:30/15:00 用两点新浪实时快照差分，输出涨跌扩散、涨幅中位数、成交额增量、4 个宽基、申万二级强弱与个股波动并推钉钉；全市场≥4000 且覆盖≥95%、行情当日且≤10分钟陈旧，上一槽位缺失=`partial` 明示未计算。报告先原子落 `data/reports/intraday-summary/`，pending outbox 落 `data/runs/intraday-summary/state.json`，成功才记 sent；不写 SQLite/计划层/持仓/关注池 |
...
| `market-tasks` | `python main.py ma-breakout daily [--date YYYY-MM-DD] [--windows 5,10] [--leader-lookback-days 60] [--top-n N] [--dry-run] [--no-push] [--json]` | 4日均线二波尾盘观察池：工作日 14:50 单次实时快照，且只允许上海当日 14:45～15:00 执行；先从 `leader_tracking` 取近 60 自然日人工确认历史龙头/最票宇宙（`trend_leader_pool` 不作默认来源），历史 9 个完成交易日取 `get_market_daily_quotes`，目标日用 `get_realtime_quotes` 的实时价和累计成交额合成临时 bar（新浪元→Tushare千元），再复用原 MA4 重新拐头 + 成交额 MA5/MA10 突破 + 快照时未涨停条件。实时行情日期必须为当日、最多陈旧10分钟；全批失败重试1次，缺票补取1次；全部不可判=`source_failed`，部分缺失/陈旧=`partial`。按累计成交额降序落 `data/reports/ma-breakout/YYYY-MM-DD.{md,json}` 并推钉钉，正文标 `[判断·实时快照]`、尚未收盘确认；`--no-push` 落盘不推，`--dry-run`/`--json` 不落不推；非当日/非时间窗/非交易日均跳过，不再回退上一交易日、不再运行21:35收盘版；不写交易计划/关注池/SQLite |
...
`intraday-monitor` 临时规则补充：`medicilon-below-87-65-20260907-1006` 监控美迪西 `688202.SH <87.65元`，2026-09-07～10-06（含首尾，共30自然日）有效；新浪行情、等于不触发、首次已命中提醒、持续命中去重、恢复后重新跌破提醒，到期停止该票取数和推送。仍复用现有 `check` / `e2e-test` 入口，无新命令。

`intraday-monitor` 临时规则补充：`haoxiangni-breakout-11-24-20260909-22` 监控好想你 `002582.SZ >11.24元`、`pinwo-foods-breakout-25-89-20260909-22` 监控品渥食品 `300892.SZ >25.89元`、`liangpin-store-breakout-10-17-20260909-22` 监控良品铺子 `603719.SH >10.17元`；三条均在 2026-09-09～09-22（含首尾，14自然日）有效，新浪行情、严格突破且等于不触发、首次已命中提醒、持续命中去重、回落后重新突破提醒，到期停止对应票取数和推送。调度统一改为每 3 分钟，仍复用现有 `check` / `e2e-test` 入口，无新命令。
