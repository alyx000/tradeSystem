...
| `market-tasks` | `python main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test [--rule-id RULE_ID] --input-by USER --confirm-real-push [--json]` | 可扩展盘中实时阈值监控与每 5 分钟 launchd；长期规则监控新浪实时 `000001.SH` 上证指数从 `<3955` 站上 `>=3955`；2026-08-19～20 日期限定规则监控金健米业 `600127.SH`、红四方 `603395.SH`、京粮控股 `000505.SZ`，用新鲜行情 `pre_close` 按交易所规则动态计算当日涨停价，盘中严格低于只提醒当前未封涨停并注明最终以收盘为准，本机与行情时间均达到15:00且仍低于才额外确认为断板；14:59行情不冒充收盘，`15:01-15:05` 仅补取动态规则终态，阶段进入事件ID且终态入账后才推进确认；等值不触发、首次已低于会推、持续命中去重、回封后再开板可重推，有效期外在 provider 前排除，前收盘价非法则 fail-closed，三只目标部分不可判为 `partial` 且 CLI 非零退出；科创50规则保持下线。保留只读交易日历门禁、当日且不超过10分钟行情新鲜度、pending 先落盘、失败同日重试和跨日/退役规则过期；`e2e-test` 可按 `--rule-id` 选择当前生效规则，缺少真实推送确认或规则已过期均在 provider 前拒绝，授权后才使用临时测试线且不读写正式状态；不写 SQLite、计划层、持仓或关注池 |
