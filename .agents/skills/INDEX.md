...
| `market-tasks` | `python main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test [--rule-id RULE_ID] --input-by USER --confirm-real-push [--json]` | 可扩展盘中实时阈值监控与每 5 分钟 launchd；当前启用规则仅监控新浪实时 `000001.SH` 上证指数从 `<3955` 站上 `>=3955`。金健米业 `600127.SH`、红四方 `603395.SH`、京粮控股 `000505.SZ` 三条断板规则已于 2026-08-20 下线，不再请求行情或推送；科创50规则保持下线。保留固定/动态阈值、盘中/收盘阶段、有效期过滤、同代码去重、只读交易日历门禁、当日且不超过10分钟行情新鲜度、pending 先落盘、失败同日重试和跨日/退役规则过期；`e2e-test` 可按 `--rule-id` 选择当前生效规则，缺少真实推送确认或规则已过期均在 provider 前拒绝，授权后才使用临时测试线且不读写正式状态；不写 SQLite、计划层、持仓或关注池 |
