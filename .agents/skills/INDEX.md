...
| `market-tasks` | `python main.py intraday-monitor check [--dry-run] [--json]` / `intraday-monitor e2e-test [--rule-id RULE_ID] --input-by USER --confirm-real-push [--json]` | 可扩展盘中监控与每 5 分钟 launchd（另有 09:59 最后补扫）；长期单标的规则监控上证指数从 `<3955` 站上 `>=3955`，横截面规则在 `09:30<=quote_time<10:00` 扫描最新价达到正式涨停价且累计成交额 `>=100亿元` 的 A 股，同股当日只推一次，覆盖板块/ST以及沪深前五开放日、北交所首日无涨跌幅限制口径；部分行情不得伪装完整空池。历史临时规则与下线规则保持既有语义；两类状态均 pending 先落盘、失败同日重试、跨日过期，runtime 切换 guard 同时保护；不写 SQLite、计划层、持仓或关注池 |
