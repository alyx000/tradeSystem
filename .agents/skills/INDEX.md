...
`intraday-monitor` 临时规则补充：`fangsheng-reach-11-11-20260903-16` 监控方盛制药 `603998.SH >=11.11元`，2026-09-03～09-16（含首尾，14自然日）有效；新浪行情、等于触发、首次已命中提醒、持续命中去重、回落后重新触达提醒，到期停止该票取数和推送。仍复用现有 `check` / `e2e-test` 入口，无新命令。
