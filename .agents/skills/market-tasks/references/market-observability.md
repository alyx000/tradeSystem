...
python3 main.py sector-crowding report --date 2026-07-17             # 只读:三部分全景+历史分位+双高+板块标签(现算)
python3 main.py sector-crowding trend --sector 801080.SI --days 60   # 只读单板块序列(建议用申万代码查询)
python3 main.py sector-crowding backfill --start 2019-01-01          # 一次性历史回填(fail-closed)
```

- **每日板块趋势标签（申万二级）**：复用 `sector_crowding_daily` 的 `close/amount_billion` 历史事实，按目标日读取时现算，不新增表、不把派生值写回快照：
  - `半年线上`：当日板块指数收盘严格高于 `MA144`（最近 144 个交易快照收盘均值），属 `[事实]`；不足 144 个完整交易快照或窗口内缺收盘时为 `null/数据不足`。
  - `年线上`：当日板块指数收盘严格高于 `MA233`（最近 233 个交易快照收盘均值），属 `[事实]`；不足 233 个完整交易快照或窗口内缺收盘时为 `null/数据不足`。
  - `近期价量共振`：最近 10 个交易快照日内，至少有一天板块指数收盘与成交额**同日**严格突破各自此前 20 个交易快照日高点，属 `[判断]`；响应保留最近共振日及两组突破值作为 `[事实]` 证据。最近窗口不能完整评估且没有已确认事件时为 `null/数据不足`，不得伪装成 `false`。
  - 当日采集以 `index_classify.is_pub` + 单码目标日前后行情构造目标日申万 L2 as-of 宇宙：当前发布码缺失、或历史有效期内缺失均 fail-closed；尚未生效/已经退出的合法空窗不误报，也不把 `is_pub=0` 的退役码当成当前应到码。分类表须经双读集合一致及已验证基线（SW2021 L2 总码≥134、当前发布码≥120）门禁，稳定部分返回也不落库。完整快照把可信 `l2_expected_codes`、catalog 数量与 `l2_universe_basis` 随元数据保存。读取时优先使用目标日可信 as-of 宇宙，允许合法新增、退出和换码；仅遗留/手工快照无元数据或元数据与实际覆盖不一致时才继承最近可信宇宙，若窗口内全程无可信元数据则按历史观察并集保守兜底，防连续部分返回逐日缩小清单。部分缺失返回 `available:true/status:partial`；目标日 L2 全缺返回 `available:false/status:missing_l2`，但仍保留预期板块清单的审计项；所有缺失项三类标签均为 `null`，不得以缩小后的清单伪装 `success`。
  - 历史代码有效期按 `sw_daily` 首末可观察行情边界推断；`index_classify` 不提供生效/退出日期，因此“真实首个有效日恰好缺数”与“尚未生效”、“真实最后有效日恰好缺数”与“已经退出”在事实源上不可区分。系统会重试空响应并记录 `l2_universe_basis`，但不把这个推断边界包装成独立官方生效日期。
  - 报告只列命中板块并给汇总（部分覆盖时显式告警）；完整 L2 清单经只读 API `GET /api/market/sector-labels/{date}` 在八步复盘「2.板块」展示，支持全部 / 半年线上 / 年线上 / 近期共振 / 年线+共振筛选。该端点使用 SQLite `mode=ro` 专用连接，请求期不执行迁移；无目标日快照返回 `available:false`，不触发采集或写入。
...
- **派生指标不落库、读取时现算**：`sector_crowding_daily` 一天一行 JSON 快照只存原始事实（close/amount/share_pct）；历史分位、双高清单及每日板块趋势标签一律在读取时从自身历史序列滚动计算——避免回填/修正历史后已落库的派生值过期。
...
- **回填 fail-closed**：`backfill` 用经自然日完整性校验的上交所 `trade_cal` 开放日作独立脊柱；任一行业码失败、响应错码/越界/落在非开放日、**可判定有效期内**整段空返回，或首端/内部/末端缺交易快照日均整体中止不落库，因而全体 L2 同日缺失也不能随行情日期并集一起漏检。区间前后行情探针结合 `is_pub` 识别尚未生效与已经退出的合法空窗（空响应须连续 3 次稳定，重跑即重试，UPSERT 幂等）。每个历史日把按上述可观察边界得到的 L2 保存为 as-of 宇宙；单片返回恰 2000 行视为镜像静默截断直接报错；两阶段流程（按码分片采集→内存按日期聚合→整日一次性 UPSERT）防同日互相覆盖；默认 2019-01-01 起（覆盖白酒极值基准）。
...
- 指数范围：上证、深成、创业板、科创50、通达信微盘股880823与平均股价；报告同时呈现成交额近20日地量分位、跌停家数和涨跌家数。
- 通达信专路：日线 `880823.TDX`（微盘股）/`avg_price`（平均股价）固定调用 tdx，要求完整800根有效且日期唯一的日线并覆盖请求窗口起点，再按日期截取；浅历史节点继续尝试下一节点，全部不足返回 `insufficient_depth`；节点按目标代码有效K线选择，失败不换指数、不补零。历史中证2000保留原名与代码；不重写归档。微盘股日线成交量仅作同源相对比较，绝对单位未核实。
