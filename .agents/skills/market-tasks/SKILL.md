...
2026-09-16：有研硅（688432.SH）监控已按用户要求下线，`youyan-silicon-breakout-46-14-20260916-30` 从 `DEFAULT_RULES` 移除，不再为该规则取数、告警或提供 `e2e-test` 选择。历史规则定义与状态仅供审计，监控能力与其他规则保留。
...
双星新材一个月临时监控：`shuangxing-materials-breakout-12-98-20260917-1016`，新浪实时 `002585.SZ >12.98元`，严格大于、等于不触发。用户于9月16日收盘后新增，按下一交易日9月17日起至2026年10月16日（首尾包含，30自然日）有效，10月17日起停止该规则取数与推送；交易日盘中每3分钟检查，首次已突破提醒、同日持续命中去重、回落至阈值或下方后再次突破提醒。复用日历/新鲜度门禁、pending/sent状态和钉钉，福龙马14.36与有研硅下线状态不变，不写持仓、关注池或计划层。
