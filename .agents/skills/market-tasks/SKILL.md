...
工作日 14:30 per-task launchd（`com.alyx.tradesystem.tail-scan`），不进 `schedule`/APScheduler；休眠期间不触发（需盘中 mac 唤醒）。也可手动执行：
...
- **候选筛选（[事实]）**：全市场实时快照（`get_realtime_quotes`，sina 分片源单点脆弱，14:30 单次触发失败自动重试一次，仍失败落 `source_failed`）→ 涨幅 > `--min-pct`（默认 7%）∩ 非ST ∩ 成交额 > `--min-amount`（默认 20 亿）。
...
- **粗权重分**：仅用于 PK 强池截断（`PK_POOL_MAX=12`）与排序破平，**不进 PK prompt**（PK 只喂 [事实] 字段，镜像 board-break）。
...
- **输出**：候选清单（按 PK 名次 / 粗分排序，全标 `[判断]`）+ 数据时效声明（涨幅/成交额/尾盘强度为实时快照，逻辑/板块为 T-1 口径，盘中形态为单次快照代理无分时数据）；MD 落盘 `data/reports/tail-scan/` + 钉钉。
- **三档运行**：裸 `daily`（落盘+推）/ `--no-push`（落盘+仅打印）/ `--dry-run`（只打印，不落盘不推送）。
