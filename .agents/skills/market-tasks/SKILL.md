...
- **确认写入护栏**：`confirm` 复用提案层的 Unicode 空白压缩板块键，并在开启事务前重新校验最多 15 只、股票规范身份全局唯一、同板块同属性唯一；旧确认稿若不满足新约束会显式拒绝，不做静默裁剪。合法 `stock_code` 以裸 6 位代码写入第 5 步并由 `leader_tracking` 优先作为身份；仅旧 payload 缺失代码时回退展示名称。首次遇到同股、同规范板块属性的旧名称型 tracking 行时，在同一事务内迁移或与既有代码行合并，避免历史分叉。
