...
`main.py post` 复用已启用的交易日20:00盘后调度，在 `post-market.yaml.raw_data.ipo_effect` 与既有 `daily_market` 保存统计、完整次新成分和来源收据，盘后 Markdown 展示同日统计。不新增独立任务、业务表或计划/关注池写入。

- 上市日龄按目标日减上市日的自然日差计算，分1～365天、1～183天、1～90天，均含边界；沪深北A股剔除ST/退市/B股和上市首日，各组嵌套不能相加。另列一年内剔除上市前5个开放日、一年内仅沪深两组。
- 统计等权涨跌均值/中位、上涨率、≥5%与≤-5%占比、开盘至收盘表现及中位相对全市场差（pp）。开收盘统计不代表可实现交易收益，不自动输出强弱评级或买卖建议。
- `get_market_daily_quotes` + `get_stock_universe_as_of` + 本批完整 `get_stock_st` 为事实来源；日历只读SSE且最近21自然日完整、目标日为开放日、上海当日16:00后才取数；非交易日/未收盘为 `skipped`，日历未知失败关闭。
- 全市场有效日线≥4000、有效率及目标日宇宙行情覆盖≥98%，代码唯一且行情100%匹配证券身份，日线日期必须精确匹配。身份/上市日期/ST硬源失败=`source_failed`；部分行情/开盘价缺口=`partial`，逐组保存应有/有效样本及缺失代码，停牌未确认不当作空集，不补0。
- 仅增加未来标准盘后采集；不直接改写历史归档，不用当前证券简称声称已恢复历史简称快照。
...
python3 main.py trend-leader daily --main-line l2                                # 仅稳定申万二级主线
python3 main.py trend-leader daily --main-line l2+concept --top-concepts 8       # 机械 ∪ 同花顺概念分支（不经 LLM）
...
- **自动申万主线稳定门**：以目标日有效集中度快照为基准，向前读取最近最多 3 个有效 `daily_volume_concentration` 快照，每个快照只取 Top-K。空 `sector_summary` 或全部为 `UNCLASSIFIED`/未分类的记录无效，不占窗口；有 2～3 个有效快照时板块须至少命中 2 次，只有 1 个时命中 1 次，没有有效快照时自动申万主线为空。结果只保留目标有效快照 Top-K 内的稳定板块并沿用其排名；目标日记录无效时回退最近有效来源日。`--sectors` 手工板块按输入顺序直接保留，不受持续性门槛影响。
...
# 只读影子监控：默认只打印，不写库、不改池、不推送
python3 main.py monthly-pattern monitor
python3 main.py monthly-pattern monitor --date 2026-07-24 --json
python3 main.py monthly-pattern monitor --max-seeds 20 --save-report

# 日频自动监控：每天落本地快照，只在完整基线之间发生状态变化时推送
python3 main.py monthly-pattern monitor-daily
python3 main.py monthly-pattern monitor-daily --dry-run
python3 main.py monthly-pattern monitor-daily --date 2026-07-24 --no-push --json
...
python3 main.py monthly-pattern facts-backfill --date 2026-07-24 --months 48 --input-by codex --dry-run
# 用户确认上条收据后，才可把完全相同的哈希用于真实派生事实写入：
python3 main.py monthly-pattern facts-backfill --date 2026-07-24 --months 48 --input-by codex --expect-receipt-hash <dry-run-receipt-hash>
```

- **完成月与覆盖边界**：只有交易日历确认的该月最后开放日不晚于扫描日时，才把该月纳入；不把仍有后续开放日的月 K 当确认信号。全市场月线来自 `get_market_monthly_quotes`，与 `adj_factor` 合并后统一前复权；历史窗口至少 35 个完成月，默认 48。每月覆盖分母必须来自外部股票基础资料 `L/D/P` 全状态，并按 `list_date/delist_date` 还原该月 as-of 应有宇宙；行情与复权覆盖通过门禁后写 `monthly_pattern_bar_manifests` 的 certified 收据，缓存复用和扫描都必须同时验证该收据，不能只凭已有月线行数猜测完整。供应商因证券代码迁移同时返回新旧影子代码时，仅在同交易所、完整且语义有效的月线九字段与复权因子一致、且唯一对应到宇宙内 canonical code 时抑制重复，并把字段值、因子与映射写入版本化 manifest 收据；缓存复用会校验九字段收据摘要，并把持久化的 OHLC/成交量/成交额六字段与复权因子逐项对照 canonical bar，schema、摘要、计数或持久化事实任一损坏均按 cache miss 重采。同一 code 在窗口内不得兼任 alias 与 canonical，缺字段、跨市场、角色反转、链式映射或歧义仍 fail-closed。本层不拼接分段行情，也不迁移旧池 episode。
...
- **日报重点漏斗与评分**：日报先保留全量技术候选计数，再只让已有 `fundamental_verified` / `active` 且财务证据为 verified 的股票进入 100 分综合评分；同股多策略先去重，技术共振 30、主线板块 25、基本面质量 20、行业内估值 10、分行业合同负债/研发投入 10、数据完整度 5。估值只读目标日前最近一批成功 `raw_interface_payloads.daily_basic`，最多允许陈旧 7 天且市场覆盖不少于 90%；金融行业优先 PB，其他行业优先正 PE(TTM)，缺失时依次回退 PS(TTM)/PB，只在同申万二级+同指标且样本不少于 5 的组内计算分位。合同负债与研发投入按行业适用性给分，`不适用` / 缺失只标资料边界，不按基本面失败处理。报告按申万二级聚合展示综合分降序 Top10，并从无 risk、行业明确、数据完整度至少 4 分的股票中给全局 Top3；综合评分只改变报告重点层，不写回池状态、不生成计划或买卖建议。
...
- **原始五表 + 派生两表**：`monthly_pattern_bars`（完成月原始 OHLCV + 复权因子）、`monthly_pattern_bar_manifests`（通过外部 as-of 宇宙覆盖门禁的 certified 月线收据）、`monthly_pattern_financial_snapshots`（公告日 as-of、按内容版本追加的财务事实）、`monthly_pattern_runs`（complete/partial/failed 运行回执）、`monthly_pattern_pool`（策略 episode 与状态）；`monthly_pattern_derived_month_facts` 独立保存经日线+复权因子认证的 `certified_bar` 或有证据的 `certified_no_trade`，`monthly_pattern_derived_fact_runs` 追加保存手工回补收据。派生层只覆盖计算视图，不覆盖原始月线五表。
...
- **派生事实回补确认流**：`facts-backfill --date ... --input-by ... --dry-run` 仅在 SQLite 内存副本规划当前真正 blocked 的缺最新月、最近 20 根已有月 K 跨度内的全部内部缺口和最近 6 月形态不可认证项；同票已知的缺口与形态异常在同一收据合并，不抓已由其他硬门排除的 `shape_short_circuited_not_matched`，也不抓确定性的 `insufficient_history`。选股宇宙为 A 股，stock_basic certified 总分母仍先逐项验收，但沪深 B 股代码 `200/201/900` 单列 `excluded_b_share_universe`，不得伪装成 A 股月线缺口；五个 A 股主分类与种子数必须生产时守恒，否则监控直接 blocked。同票合并为一次日线和一次复权因子区间请求；日线原生 `vol=手`、`amount=千元` 必须分别换算为月线事实的 `股`、`元` 后再做有界舍入交叉验证，已有 raw 的月末复权因子也必须与日线锚因子一致；同日冲突、因子缺失/非法、超出舍入界的月线冲突均保留 unresolved。整月无日线、通过 as-of 宇宙覆盖门的全市场月线源也无该票且该票在册时，只能记 `certified_no_trade`，不得伪造平盘月 K；证据哈希必须绑定该月全市场月线与历史宇宙的完整排序行摘要，不能只绑定计数。dry-run 以严格只读连接复制真实库到内存，partial 也生成内存预览；排序收据的 `receipt_hash` 绑定 raw/manifest/既有派生事实水位，真实写入必须重算得到相同 `--expect-receipt-hash`，且 unresolved、截断与事实漂移均为 0，并在 `BEGIN IMMEDIATE` 锁内再次复核。确认通过后只在同一事务内执行派生两表专用 schema ensure，表、索引与防改触发器均按 SQL 指纹验签；禁止调用全库 migrate，DDL、运行收据和事实行共同提交或回滚。确认范围是派生事实与输入水位，`monitor_preview` 仅为信息性预览、不进入写入确认哈希。该命令仅手工审计回补，不挂 launchd、不推送、不写 pool/watchlist/TradeDraft/TradePlan。
- **日频影子监控 `monitor`**：只读取目标月之前连续的 certified 完成月事实，并在计算视图中优先采用验签通过的 `certified_bar`，绝不覆盖 raw 表；有 raw 月线时 raw 自动压过旧 `certified_no_trade`。最新完成月为回踩月，其前紧邻连续阳月 `>=5`，月 `MA5>MA10>MA20` 且 `low<=MA5<=close` 才形成月线种子，回踩月收阴只作偏好证据。最新完成月的 `get_stock_universe_as_of` 必须与 certified manifest 的 `universe_count` 精确一致，再用于区分已合法退出的历史代码与本应存在的股票；后者必须实际覆盖本轮全局最新完成月，即使整个 48 月窗口都没有 bar 也要计缺失，不能拿旧后缀冒充当月种子。验签通过的 `certified_no_trade` 只计 `evidenced_not_applicable`，不合成价格、不进候选，也不再冒充 unresolved。仅对种子逐票拉截至目标日的 `get_stock_daily_range` + `get_stock_adj_factor_range`，以目标日为统一前复权锚；在同一 T 日坐标下，以前四个连续自然月月末收盘 + 当前月 T 日收盘计算动态 MA5，`current_close >= dynamic_ma5` 才保留在当前观察清单。目标月只作 `as-of` 当前资格硬门，不参与新增完成月种子；目标日收盘低于动态 MA5 的历史种子移出主清单，单列 `waiting_monthly_reclaim` 等待后续日频重算，且不得因日/周 MACD 已共振而晋级。随后计算日线与严格 as-of 周线 MACD(12/26/9)，把 `DIF/DEA` 双线零轴上与 `DIF>=DEA` 分开，并识别种子月后首次零轴重回；5/13 均量线只标“现有系统辅助口径”，不冒充本次转写已确认参数，也不作为状态硬门。未显式传日期时按上海时间 15:30 收盘安全线选择最近已完成开放日；目标日 ST 身份独立读取 `get_stock_st`，成功空表或非法代码同样 fail-closed，简称失败只影响展示，不得让可选申万行业源变成技术硬门。显式历史日不使用当前行业映射；最近已完成开放日可展示当前申万快照，但主线稳定前排始终只作 `[判断]` 优先级、不是硬剔除。关键月线收据、最新月 as-of 宇宙、目标日 ST 身份、目标日日线或复权因子缺失时标 `blocked/partial`，不得伪装成正常空候选。`monitor` 使用 SQLite `mode=ro`，默认只打印，`--save-report` 才落 `data/reports/monthly-pattern-monitor/`；不写原始五表、派生两表或观察池，不要求 `--input-by`、不推钉钉、不挂 launchd，供手工 walk-forward 与现况核验。
- **每日自动层 `monitor-daily`**：复用上述只读计算，但禁止 `max_seeds` 截断；独立 launchd `com.alyx.tradesystem.monthly-pattern-monitor` 每 15 分钟轻量 tick，runner 只在上海工作日 19:10（含）至 19:25（不含）执行一次重任务，避免 Mac 本机时区变化造成漂移；不进 `main.py schedule` / APScheduler。默认运行必须满足“未显式传 `--date` 且最近已收盘开放日=上海自然日今天”，周末、节假日和盘中安全跳过；初始化、交易日历或只读库故障仍落 `blocked` 健康收据，其中日历未确认时只写本地并保留 pending，绝不越过交易日推送闸门。显式历史 `--date` 默认只预览、不落文件且永不推送；只有同时带 `--no-push` 才保存并推进基线。完整 JSON latest 快照落 `data/runs/monthly-pattern-monitor/snapshots/YYYY-MM-DD.json`，Markdown latest 报告落 `data/reports/monthly-pattern-monitor/YYYY-MM-DD.md`，每次实际写入另在 `data/runs/monthly-pattern-monitor/attempts/YYYY-MM-DD/` 原子追加不可覆盖的 `planned` / `delivery` / `final` JSON，保留完整事件与发送结果；`state.json` 只作完整基线、pending outbox 和 sent/suppressed 去重水位，不是业务池。只有同一 `seed_month_end` 的 `complete -> complete` 才比较股票状态；`partial/blocked` 只报运行健康且不推进股票基线，健康指纹包含所有导致 partial 的缺口股票身份与原因（含 `insufficient_history`）；首个完整快照只初始化基线，完成月翻页只报 rollover 并重建基线，禁止批量伪造进出事件；同日状态哈希重跑幂等，名称/行业/主线标签及数值证据变化不触发状态事件。动态 MA5 收复/失守、日/周 MACD 双线零上与零上运行、日线重回和日周共振等变化才进入通知候选；通知发送前先原子落 pending，钉钉成功后才记 sent，失败下次按事件流 at-least-once 重试（即使随后状态反转，未送达的历史变化仍保留）。`--no-push` 会保存快照并明确抑制本轮新通知，`--dry-run` 不写文件、不推送。全程 SQLite `mode=ro`，不写原始五表、派生两表、`monthly_pattern_pool`、关注池、TradeDraft 或 TradePlan。
- **完成月种子三态短路**：最近 20 个完成月的连续性与合法 close 仍优先 fail-closed；最近 6 个月形态窗中，声明为有效的 OHLC 结构同样优先 fail-closed。因月内复权无法认证而出现的 `price_shape_valid=False` 只把对应形态硬门记为未知，但其 open/high/low 仍须存在、有限且为正；若均线排列、`close>=MA5`、已知阴月或已知 `low` 等任一可信硬门已明确失败，整票直接归 `not_matched`，并计 `shape_short_circuited_not_matched`；只有没有已知失败且最终结论确实依赖未知形态时，才计 `blocked_price_shape`。三态短路不得把原 blocked 股票直接变成 matched。
- **动态硬门三态 fail-closed**：`support_held=True` 才进入 `candidates`；`False` 进入 `waiting_monthly_reclaim`；月份不足、证据缺失或旧结构无法得出布尔结论时进入 `indeterminate_current_month_ma5`，运行至少为 `partial`，不得把未知项展示在动态 MA5 门内。
- **跨月边界**：`waiting_monthly_reclaim` 是无状态、同一目标月内的日频计算结果；自动层只把完整快照保存为通知基线，月度翻页即整体替换基线，绝不把旧等待项持久化成跨月业务 episode。课程提到“下个月跌破后再等收回”，但没有给等待 episode 的保留/失效期限，不得擅自扩成永久池。
...
长春燃气均线附近监控：`changchun-gas-near-ma20-20260914-22`，新浪实时 `600333.SH` 价格进入动态前复权 MA20 的 ±1% 区间（含边界）时提醒。MA20=(前19个已收盘开放日的前复权收盘价之和+当日最新价)/20，历史数据按实时昨收锚定到当前价格坐标；同日缓存复用历史、每次最新价更新均线，样本日期或昨收锚变化重新取数，日历/日线/因子/实时行情缺证据 fail-closed。有效期为 2026-09-14～09-22 的7个交易日（14、15、16、17、18、21、22日），盘中每3分钟检查；首次已在区间内提醒、持续在区间内去重、离开后再次进入提醒，9月23日起停止此规则。复用现有 pending/sent 状态与钉钉，不写持仓、关注池或计划层。

长春燃气 MA5 重新站上监控：`changchun-gas-reclaim-ma5-20260914-22`，与上述 MA20 规则独立并存；动态前复权 MA5=(前4个已收盘开放日收盘价之和+当日最新价)/5，沿用相同复权锚与缺源守卫。仅同日观测从价格≤MA5转为价格>MA5时推钉钉，相等不触发；每天首次有效采样只建基线，已在线上或隔夜跳空站上不补报，持续在线上去重，回落至不高于均线后再上穿可重推。有效期2026-09-14～09-22（7个交易日），沿用盘中每3分钟检查，9月23日起停止。

福龙马临时突破监控：`fulongma-breakout-14-36-20260916-24`，新浪实时 `603686.SH >14.36元`（严格大于，等于不触发）；有效期2026-09-16～09-24首尾包含，共7个开放交易日（16、17、18、21、22、23、24日），9月25日起停止。沿用盘中每3分钟、首次已突破提醒、同日持续命中去重、回落至阈值或下方后再次突破提醒；复用原有日历/新鲜度守卫与钉钉，其他监控不变。 2026-09-16将阈值从14.15调整为14.36，使用新规则身份避免继承旧命中状态；旧规则退出默认监控，原有效期不延期。

2026-09-16：有研硅（688432.SH）旧46.14元监控已按用户要求下线，`youyan-silicon-breakout-46-14-20260916-30` 从 `DEFAULT_RULES` 移除，不再为该规则取数、告警或提供 `e2e-test` 选择。历史规则定义与状态仅供审计，监控能力与其他规则保留。

欣旺达7个交易日临时监控：`sunwoda-breakout-19-94-20260917-28`，新浪实时 `300207.SZ >19.94元`，严格大于、等于不触发。有效期2026-09-17～09-28首尾包含，共7个开放交易日（17、18、21、22、23、24、28日），周末与中秋休市不计入，9月29日起停止该规则取数与推送。沿用盘中每3分钟、首次已突破提醒、同日持续命中去重、回落至阈值或下方后再次突破提醒，复用日历/新鲜度守卫与钉钉；其他监控、历史状态不变，不写持仓、关注池或计划层。

双星新材一个月临时监控：`shuangxing-materials-breakout-12-98-20260917-1016`，新浪实时 `002585.SZ >12.98元`，严格大于、等于不触发。用户于9月16日收盘后新增，按下一交易日9月17日起至2026年10月16日（首尾包含，30自然日）有效，10月17日起停止该规则取数与推送；交易日盘中每3分钟检查，首次已突破提醒、同日持续命中去重、回落至阈值或下方后再次突破提醒。复用日历/新鲜度门禁、pending/sent状态和钉钉，福龙马14.36与有研硅旧46.14元规则下线状态不变，不写持仓、关注池或计划层。

2026-09-17大金重工阈值替换：旧35.95元规则退出默认监控与真实测试入口，历史定义及状态仅供审计。新规则 `dajin-heavy-breakout-41-96-20260917-28` 监控新浪实时 `002487.SZ >41.96元`（严格大于，等于不触发），使用独立规则标识避免继承旧命中状态。有效期重新设为2026-09-17～09-28首尾包含的7个交易日（17、18、21、22、23、24、28日），周末及中秋休市不计入，9月29日起停止此规则取数与推送。沿用盘中每3分钟、首次已突破提醒、同日持续命中去重、回落至阈值或下方后再次突破提醒；其他监控不变。

飞龙股份10个交易日临时监控：`feilong-breakout-57-16-20260917-1008`，新浪实时 `002536.SZ >57.16元`（严格大于，等于不触发）。有效期2026-09-17～10-08首尾包含，共10个开放交易日（9月17、18、21、22、23、24、28、29、30日和10月8日），周末、中秋及国庆休市不计入，10月9日起停止该规则取数与推送。沿用盘中每3分钟、首次已突破提醒、同日持续命中去重、回落至阈值或下方后再次突破提醒，复用日历/新鲜度守卫与钉钉；其他监控及历史状态不变。

飞龙股份3个交易日均线上穿监控：`feilong-reclaim-ma5-20260921-23`，新浪实时 `002536.SZ` 从不高于动态前复权MA5变为严格高于时提醒，等于不触发；MA5=(前4个已收盘开放日的前复权收盘价之和+当日最新价)/5，Tushare日线及复权因子按实时昨收锚定当前价格坐标。有效期2026-09-21～09-23首尾包含，共3个交易日，9月24日起停止该均线规则的历史取数与推送。每日首次有效采样只建基线，已在线上或隔夜跳空不补报；持续在线上去重，回落至不高于均线后再次上穿可重推。原57.16元突破监控继续独立运行至10月8日，同股行情复用；沿用盘中每3分钟、日历/新鲜度/历史覆盖守卫及钉钉，缺证据不建基线、不发提醒，不改历史状态和其他监控。

2026-09-23：科翔股份（300903.SZ）108.30元监控已按用户要求提前下线，`kexiang-below-108-30-20260922-1008` 从 `DEFAULT_RULES` 移除，不再为该规则取数、告警或提供真实 `e2e-test` 选择。原有效期9月22日至10月8日仅作历史审计，规则定义与历史状态保留，不再表示启用；监控能力、每3分钟调度及其他标的保持不变。

中材科技3个交易日临时监控：`zhongcai-tech-below-59-00-20260924-29`，新浪实时 `002080.SZ <59.00元`（严格小于，等于不触发）。9月24日开盘前新增，有效期2026-09-24～09-29首尾包含，覆盖9月24、28、29日三个开放日，中秋及周末休市不计入；9月30日起停止该规则取数与推送。沿用盘中每3分钟、首次已跌破提醒、同日持续命中去重、回升至阈值或上方后再次跌破提醒，复用日历/新鲜度守卫与钉钉；科翔股份保持下线，其他监控及历史状态不变。

有研硅7个交易日新阈值监控：`youyan-silicon-breakout-57-96-20260922-1008`，新浪实时 `688432.SH >57.96元`（严格大于，等于不触发）。9月21日收盘后新增，从下一交易日起计满7天：有效期2026-09-22～10-08首尾包含（9月22、23、24、28、29、30日和10月8日），周末、中秋及国庆休市不计入，10月9日起停止该规则取数与推送。沿用盘中每3分钟、首次已突破提醒、同日持续命中去重、回落至阈值或下方后再次突破提醒，复用日历/新鲜度守卫与钉钉；旧46.14元规则仍不在默认监控或真实E2E选择中，历史定义与状态保留，其他监控不变。

南华生物单日断板监控：`nanhua-bio-board-break-20260923`，新浪实时 `000504.SZ`，仅2026-09-23一个交易日有效，9月24日起停止该规则取数与推送。复用 `threshold_mode=daily_up_limit`：依据实时昨收及证券板型计算当日涨停价（分位四舍五入，不写死价位），最新价严格低于该线时提醒，等于不触发；首次未封板提醒，持续未封板去重，回封后再开板可重推。盘中仅标注“当前未封涨停，最终是否断板以收盘为准”；收盘仍未封板另发一次断板确认，必须取得不早于15:00的终态行情，可由既有15:01～15:05补采窗口完成。沿用盘中每3分钟、日历/新鲜度守卫及钉钉；昨收或终态行情缺失不误报，其他监控和历史状态不变，已下线的金健米业/红四方/京粮控股不恢复。

新增临时规则 `medicilon-below-87-65-20260907-1006`：新浪实时 `688202.SH` 美迪西最新价格严格 `<87.65元`，有效期为 2026-09-07 至 2026-10-06（首尾包含，共 30 个自然日，交易日盘中执行）。等于不触发；首次观察已在线下会提醒，同日持续命中去重，恢复到线上或等于阈值后再次跌破可重推；10月7日起不再抓取或推送该规则。其余规则、交易日历、新鲜度、独立状态与调度边界不变。

2026-09-20方盛制药监控续期：旧规则 `fangsheng-reach-11-11-20260903-16` 已到期并退出默认监控与真实测试入口，历史定义和状态保留供审计。新规则 `fangsheng-reach-11-11-20260921-1012` 从下一交易日2026-09-21重新计满10个交易日，至2026-10-12（首尾包含；9月21、22、23、24、28、29、30日及10月8、9、12日），不追补9月17、18日。仍监控新浪实时 `603998.SH >=11.11元`，等于触发；首次已命中提醒、同日持续命中去重、回落线下再触达提醒。10月13日起停止该规则取数与推送；使用独立规则标识，不修改旧命中记录，沿用盘中每3分钟及现有钉钉、日历、新鲜度守卫，其他监控不变。

新增三条两周临时规则：`haoxiangni-breakout-11-24-20260909-22`（新浪实时 `002582.SZ` 好想你严格 `>11.24元`）、`pinwo-foods-breakout-25-89-20260909-22`（`300892.SZ` 品渥食品严格 `>25.89元`）、`liangpin-store-breakout-10-17-20260909-22`（`603719.SH` 良品铺子严格 `>10.17元`）。三条有效期均为 2026-09-09 至 2026-09-22（首尾包含，共 14 个自然日，交易日盘中执行）；等于不触发，首次观察已在线上提醒，同日持续命中去重，回落到阈值或下方后再次突破可重推；9月23日起不再抓取或推送对应规则。

独立 launchd `com.alyx.tradesystem.intraday-monitor` 每 3 分钟做一次无时区 tick，并额外固定在本机 09:59 补扫一次（runner 仍以 Asia/Shanghai 二次门禁），runner 在上海交易时段 `09:30-11:30 / 13:00-15:00` 做常规检查；Python 对 11:30/15:00 结束分钟保留不足 1 分钟的进程启动宽限，另允许 `15:01-15:05` 补取不早于 15:00 的终态行情。长期规则包括：新浪实时 `000001.SH` 上证指数从 3955 点下方站上 3955 点（`>=3955`），以及同花顺官方实时 `883421.THS` 同花顺全A（沪深）相对昨收的单日涨跌幅严格 `<-4.00%`；等于 -4.00% 不触发。横截面规则在 `09:30 <= quote_time < 10:00` 批量扫描最新价达到交易所正式涨停价且当日累计成交额 `>=100亿元` 的 A 股，同股当日只推一次。临时规则包括：2026-08-31 的 `300285.SZ` 国瓷材料严格跌破 67.22 元，以及 2026-08-31～09-02 的 `688361.SH` 中科飞测严格跌破前 5 个已收盘交易日的前复权 MA5。历史科创50/凯莱英临时规则保留审计，金健米业、红四方、京粮控股三条断板规则保持下线。
...
```bash
python3 main.py tail-scan daily
python3 main.py tail-scan daily --date 2026-07-13 --dry-run   # 只打印，不落盘不推送（历史校准）
python3 main.py tail-scan daily --no-push                      # 落盘但不推送
python3 main.py tail-scan daily --no-llm                       # 跳过 LLM 两两 PK
...
- **近30自然日催化（只读本地证据）**：`teacher_notes` 仅按标准股票代码精确命中，慧博摘要仅按股票名称精确命中，`industry_info` 基于申万二级、主营摘要、产品或已验证概念标签做受控匹配；证据分别保留 `[事实]`、`[老师观点]`、`[研报观点]`、`[来源陈述]` 边界标签。任一来源失败只降级主营或催化对应维度，不中断整批筛选与报告。
- **粗权重分**：仅用于 PK 强池截断（`PK_POOL_MAX=12`）与排序破平，**不进 PK prompt**；PK 只喂边界受控字段（产业链位置显式保留程序 `[判断]`，近期催化保留事实/观点/来源标签）。
- **LLM 两两 PK 循环赛（`--no-llm` 关闭）**：`itertools.combinations` 循环赛，180 秒预算熔断/无效场率 >50% 熔断（`status="melted"`）；候选 <2 只时 `run_pk` 自走 `status="skipped"`（不再用 `len(scored)>=2` 短路，避免与 `--no-llm` 的 `None` 分支撞车）；PK 理由段经红线过滤后渲染。
- **输出**：候选清单按 PK 名次 / 粗分排序，排序标 `[判断]`，每票显示 `[事实·主营]`、`[判断·产业链位置]` 与近期催化分层标签；数据时效声明区分实时快照、T-1 逻辑/板块、当前公开静态主营和近30自然日催化。本地 MD 在 `data/reports/tail-scan/` 保留全量；钉钉正文超长时限制为 ≤18000 UTF-8 bytes、最多前12个完整候选块，并附完整报告路径，绝不从候选块中间截断。
- **三档运行**：裸 `daily`（落盘+推）/ `--no-push`（落盘+仅打印）/ `--dry-run`（只打印，不落盘不推送）。
- **核心源失败**：不产出正常候选清单，落 `source_failed` 失败报告 + 推告警 + 非零退出（launchd 场景可观测，镜像 board-break）。
- **无池无状态**：不落库、不建观察池、不写交易计划层。
- 依赖 env 同 volume-watch（`TUSHARE_TOKEN` + 钉钉）；PK 依赖 Antigravity CLI（同 board-break/research-digest 家族），可调 env：`ANTIGRAVITY_BIN`/`AGY_BIN`、`LLM_MODEL`、`LLM_TIMEOUT_SECONDS`；红线过滤复用 `services.recommend.formatter.REDLINE_KEYWORDS`。
...
- **确认写入护栏**：`confirm` 复用提案层的 Unicode 空白压缩板块键，并在开启事务前重新校验最多 15 只、股票规范身份全局唯一、同板块同属性唯一；旧确认稿若不满足新约束会显式拒绝，不做静默裁剪。展示内股票代码支持有/无空白及 `.SH/.SZ/.BJ` 后缀；六位代码后的非法后缀、展示代码与显式 `stock_code` 冲突均 fail-closed。合法 `stock_code` 以裸 6 位代码写入第 5 步并由 `leader_tracking` 优先作为身份；仅旧 payload 缺失代码时回退展示名称。同股、同规范板块属性的旧名称型 tracking 行仅在全库及同批名称代码映射唯一时于同一事务内迁移或与既有代码行合并，避免历史误归与分叉。
...
HUIBO_HOT_REPORT_URL='<Computer Use 取得的当前 URL>' HUIBO_REPORT_PDF_DIR='/Users/alyx/tradeSystem/data/reports/huibo/downloaded/2026-06-06' HUIBO_REFRESH_URL_FROM_APP=0 HUIBO_ALLOW_DIRECT_PDF_DOWNLOAD=0 node scripts/workflows/research-digest-workflow.mjs daily --date 2026-06-06 --reader-cap 20 --reader-concurrency 20 --reader-max-attempts 2 --preflight --publish --include-base-digest
