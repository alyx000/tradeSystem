...
- **产业逻辑增强（每票展示）**：`[事实·主营]` 由 Tushare `stock_company` 主源、AkShare `stock_zyjs_ths` 补缺；这是扫描时取得的当前公开静态资料，**不是历史 as-of 快照**。`[判断·产业链位置]` 只基于申万二级、主营与产品做受控归纳，不引入模型自由发挥。
- **近30自然日催化（只读本地证据）**：`teacher_notes` 仅按标准股票代码精确命中，慧博摘要仅按股票名称精确命中，`industry_info` 仅按申万行业匹配；证据分别保留 `[事实]`、`[老师观点]`、`[研报观点]`、`[来源陈述]` 边界标签。任一来源失败只降级主营或催化对应维度，不中断整批筛选与报告。
