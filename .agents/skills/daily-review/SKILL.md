...
①各大指数同时执行[趋势、动能、量价与跨周期检查](references/index-analysis.md)，第1路必须读取该规范：保留原日线MA5/MA10表，补充MA5方向、日线MACD最后5日变化、同指数5日量价及60分钟对照；正文合并为一句综合摘要，数值与条件放折叠证据。日线MACD固定124个完整开放日预热并保留最后5日，量能不足单项未计算；国证2000分钟代理不得与通达信微盘股或历史中证2000日线合并为同指数共振。修复与转弱证据双向检查，未来条件标 `[判断]`，缺口进入 `ops`；不由死叉推导必跌或中阴线。本项为Agent生成流程要求，尚非组装器自动计算或强制硬门。
...
4. 按八步复盘法汇总：
   - 大盘分析
   - 板块梳理（机构定价视角可引用 `python3 main.py research-digest trend` 的申万一级研报覆盖占比 [事实] 计数；「哪些板块覆盖热度上行/下行」属趋势解读，提及时标 [判断]。**定位=机构议程背景，禁止当短线方向先验引用**——2026-07 实测覆盖 Δpp 与后 1~2 日板块涨跌 Spearman≈0[单期样本]，它反映卖方研究日历而非资金轮动；数据不足时跳过，不强行引用。`--json` 每行业附 `streak_up` 连续上行有效日数[滚动窗份额逐窗严格递增尾部步数]，「覆盖持续上行」只作机构关注度提升的中期观察信号[判断]（出处参照 teacher_notes#614 鞠磊「医药研报覆盖显著增加是机构关注度提升的观察信号」），同样不作短线先验）
   - 情绪周期
   - 风格化赚钱效应
