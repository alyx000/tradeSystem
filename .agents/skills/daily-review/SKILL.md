...
①各大指数同时执行[趋势、动能、量价与跨周期检查](references/index-analysis.md)，第1路必须读取该规范：保留原日线MA5/MA10表，补充MA5方向、日线MACD最后5日变化、同指数5日量价及60分钟对照；正文合并为一句综合摘要，数值与条件放折叠证据。日线MACD固定124个完整开放日预热并保留最后5日，量能不足单项未计算；国证2000分钟代理不得与中证2000日线合并为同指数共振。修复与转弱证据双向检查，未来条件标 `[判断]`，缺口进入 `ops`；不由死叉推导必跌或中阴线。本项为Agent生成流程要求，尚非组装器自动计算或强制硬门。
