...
```bash
make bootstrap   # 首次安装依赖 + hooks
make doctor      # 检查本机环境是否齐全
make check       # 前后端全量检查
make dev         # 一键启动前后端开发服务
make today-close # 跑今天的收盘后主流程
```
...
- Local check: `cd web && npm run check`
- Local hook: `.githooks/pre-push`
- CI workflow: [web-check.yml](/Users/alyx/tradeSystem/.github/workflows/web-check.yml)

### Scripts

- Local check: `bash scripts/check.sh`
- Local hook: `.githooks/pre-push`
- CI workflow: [scripts-check.yml](/Users/alyx/tradeSystem/.github/workflows/scripts-check.yml)
...
- 首次安装：`make bootstrap`、`make doctor`、`make hooks-install`
...
python3 main.py plan confirm --date YYYY-MM-DD --draft-id draft_xxx
python3 main.py plan diagnose --date YYYY-MM-DD --plan-id plan_xxx --json
python3 main.py plan review --date YYYY-MM-DD --plan-id plan_xxx
...
完整 `make` 目标清单见 [commands.md](/Users/alyx/tradeSystem/docs/commands.md)；如需机器可读版本，使用 [commands.json](/Users/alyx/tradeSystem/docs/commands.json)。
...
# 打开市场看板
open http://localhost:5173/market/YYYY-MM-DD

# 打开其他工作台
open http://localhost:5173/
open http://localhost:5173/search
open http://localhost:5173/commands
open http://localhost:5173/plans/YYYY-MM-DD
...
# 拉取市场摘要与盘后信封
curl http://localhost:8000/api/market/YYYY-MM-DD
curl http://localhost:8000/api/post-market/YYYY-MM-DD
```
...
- Local check: `cd web && npm run check`
- Local git hook: repo uses `.githooks/pre-push`
- CI check: `.github/workflows/web-check.yml`

To enable the local hook on a fresh clone:
